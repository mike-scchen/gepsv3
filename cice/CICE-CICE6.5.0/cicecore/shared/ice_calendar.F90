...
!           math to myear_max which is a parameter in this module.
      ...
      integer (kind=int_kind), public :: &
         ...
         npt_cpl  , & ! total number of time steps for coupler data exchange, ycshao
         ...
         stop_cpl     , & ! if 1, end cice_run in a coupler step ! ycshao
      ...
      real (kind=dbl_kind), public :: &
         ...
         dt_cpl         , & ! coupled data exchange timestep(s), ycshao
...
! the first timestep
      ...
      if (my_task == master_task) then
         ...
         write(nu_diag,'(1x,2a,i9,a,f13.2)') subname,'           coupler ',npt_cpl,' '//trim(npt_unit )//' with dt_cpl= ',dt_cpl
...
!         ttime                          ! time variable
...
#endif
      stop_cpl = 0                         ! ycshao
      if (mod(istep, npt_cpl) .eq. 0) stop_cpl = 1 ! ycshao
      ...
      if (my_task == master_task .and. mod(istep1,diagfreq) == 0 &
                                 .and. stop_now /= 1) then
        write(nu_diag,*) ' '
        write(nu_diag,'(a7,2i10,4x,a6,i10,4x,a4,i10)') &
             'istep1:', istep1, istep, 'idate:', idate, 'sec:', msec
        write(nu_diag,'(a9,3i10)') &
             'cpl step:', istep, npt_cpl, stop_cpl
      endif
