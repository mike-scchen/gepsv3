...
#=======
else if (${ICE_MACHCOMP} =~ betago*) then
cat >> ${jobfile} << EOFR
mpiexec.hydra -np ${ntasks} ./cice >&! \$ICE_RUNLOG_FILE
EOFR

#=======
else if (${ICE_MACHCOMP} =~ fx1000*) then
cat >> ${jobfile} << EOFR
/usr/bin/time -p mpiexec -n \${MPI} ./cice >&! \$ICE_RUNLOG_FILE
EOFR
