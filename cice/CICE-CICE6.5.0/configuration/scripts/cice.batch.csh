...
else if (${ICE_MACHINE} =~ betago*) then
cat >> ${jobfile} << EOFB
#!/bin/bash
#PBS -V
#PBS -N ${ICE_CASENAME}
...
echo "Cores count: " \$PBS_NP
NPROCS=\`wc -l < \$PBS_NODEFILE\`
cp \$PBS_NODEFILE MPI.nodefile
echo "Cores used: " \$NPROCS
echo "Starting at: " \`date\`
EOFB

else if (${ICE_MACHINE} =~ fx1000*) then
cat >> ${jobfile} << EOFB
...
#PJM --no-stging
#PJM --mpi "proc=${ICE_MACHINE_TPNODE}"
#PJM -j
#PJM -g sum
#PJM -N CICE_jobs
#PJM -o %j.log
