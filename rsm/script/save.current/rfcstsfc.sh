...
#yj#
ls -altr r_sigi r_sigitdt r_sfci
echo "===check the link for input===fcst"
...
ls -latr r_sig.f$hx r_sigftdt r_sfc.f$hx r_flx.f$hx r_flx.f00 r_init
echo "===check the link for output===fcst"
#
...
#
rm -f $PROG.x
#ln -fs $SRCDIR/rsm_fcst.fd/$PROG.x $PROG.x
ln -fs $EXPEXE/$PROG.x $PROG.x
###$RUNENV ./$PROG.x <rfcstparm >stdout.fcst$hx 2>&1
pwd
#$RUNENV ./$PROG.x <rfcstparm >stdout.fcst$hx
#mpiexec -stdin rfcstparm -of stdout.fcst$hx -n 128 ./$PROG.x 
export RUNENVf='/usr/bin/time -p mpiexec -stdin rfcstparm -of stdout.fcst'$hx
$RUNENVf -n ${NPES} ./$PROG.x
#
if [ $? -ne 0 ] ; then
   exit 8
fi
