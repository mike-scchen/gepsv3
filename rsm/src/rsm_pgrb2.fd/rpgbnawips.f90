...
!   THE program predices and writes a pressure GRIB1 file to UNIT 51    
...
!                                                                       
!$$$                                                                    
      PARAMETER(LEVT=6,LEVZ=3,NROW=4,MYBIT=16)
      PARAMETER(LEVMAX=100,KOMAX=100)                                   
      DIMENSION IDATE(4)                                                
      DIMENSION SI(LEVMAX+1),SL(LEVMAX)                                 
      DIMENSION AKI(LEVMAX+1),BKI(LEVMAX+1)
      DIMENSION PO(KOMAX),ZZ(KOMAX)                                     
      DIMENSION IDS(255),POT(255)
      DIMENSION IDS131(255),POT131(255)
      DIMENSION IDS133(255),POT133(255)
...
!adding to 26 layer
      DATA PO/1000.,975.,950.,925.,900.,850.,800.,750.,700.,650.,600.,  &
     &         550.,500.,450.,400.,350.,300.,250.,200.,150.,100.,       &
     &          70., 50., 30., 20., 10.,  7.,  5.,  3.,  2.,  1.,       &
     &     69*0./                                                       
!normal CWB 16 layer of pressure level
...
!    &        70.,50.,30.,20.,10.,84*0./             
      ...
      DATA IDS131/255*0/,POT131/255*0./
      DATA IDS133/255*0/,POT133/255*0./
...
!  SET DEFAULTS AND READ NAMELIST                                       
      PRINT *,' SET DEFAULTS '                                          
      CALL IDSDEF(2,IDS)                                                
      CALL IDSDEF(131,IDS131)                                                
      CALL IDSDEF(133,IDS133)                                                
      CALL CTLINI                                                       
...
!  READ FIRST SIGMA HEADER RECORD                                       
      ...
      CALL RRDSGH(NSIG,FHOUR,IDATE,SI,SL,AKI,BKI,                       &
...
!  TRANSFORM TO PRESSURE GRIB AND ATTEMPT TO READ NEXT SIGMA HEADER     
      if( KO == 0 )then
        do i=1,KOMAX
          if( PO(i) > 0.00001 )KO=KO+1
        enddo
!      else
!        itmp=0
!        do i=1,KO
!          if( PO(i) < 0.00001 ) itmp=itmp+1
!        enddo
!        if(itmp>0)then !remove zoro value
!         do i=1,KO
!           if( PO(i) < 0.00001 )then
!             do j=i,KO
!               PO(j)=PO(j+1)
!             enddo
!           endif
...
!  PRESET DIMENSION OF NFLDS FOR GRID DATA FROM RRDSGR                  
     ...
     &            NPGB,NFGB,NCPUS,MXBIT,IDS,IDS131,IDS133,              &
     &            POT,POT131,POT133,KTT,                                &
...
!-----------------------------------------------------------------------
!FPP$ NOCONCUR R                                                        
      SUBROUTINE HYDRO(IM,IX,KM,SIO,SLO,ZS,TV,Z,ZI)                      
!$$$  SUBPROGRAM DOCUMENTATION BLOCK                                    
!                                                                       
...
!                                                                       
!$$$                                                                    
      DIMENSION SIO(IX,KM+1),SLO(IX,KM),ZS(IM),TV(IX,KM)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      DO K=1,KM-1                                                       
        DO I=1,IM                                                       
          CA=ROG*LOG(SIO(I,K)/SLO(I,K))                                         
          CB=ROG*LOG(SLO(I,K)/SIO(I,K+1))                                       
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      ...
      END                                                               
!-----------------------------------------------------------------------
      SUBROUTINE LIFTIX(IM,IX,KT,PT,KM,SIO,SLO,PS,T5,TPT,QPT,T,Q,        &
...
!FPP$ EXPAND(FPKAP,FTDP,FTLCL,FTHE,FTMA)
      ...
      REAL DLS(IM,KM),SLK(IM,KM),PSK(IM)
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!  POSITIVELY BUOYANT AREA (CAPE) AND NEGATIVELY BUOYANT AREA (CIN).
      DO K=1,KM
        DO I=1,IM
        IF(SIO(I,K+1).GT.SIMIN) THEN
          DLS(I,K)=LOG(SIO(I,K)/SIO(I,K+1))
          SLK(I,K)=SLO(I,K)**RK
        ENDIF
        ENDDO
      ENDDO
...
!-----------------------------------------------------------------------
!FPP$ NOCONCUR R                                                        
      SUBROUTINE OMEGAW(IM,IX,KM,SIO,SLO,PS,PSX,PSY,D,U,V,              &
     &                  PP,TT,WW,O,OS,W,NONHYD,AKI,BKI)            
!$$$  SUBPROGRAM DOCUMENTATION BLOCK                                    
!                                                                       
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      ...
      DO K=KM,1,-1                                                      
        DO I=1,IM                                                       
          VGRADP=(U(I,K)*PSX(I)+V(I,K)*PSY(I))*(BKI(K)-BKI(K+1))
          GRADPV=VGRADP+D(I,K)*(SIO(I,K)-SIO(I,K+1))
          SUM(I)=SUM(I)+GRADPV
          O(I,K)=( (0.5*D(I,K)*(SIO(I,K)-SIO(I,K+1)))                   &
     &            +(BKI(K)*VGRADP/(BKI(K)-BKI(K+1)))                    &
     &            -SUM(I) )*PS(I)*1.E3
...
!     AKI          REAL (LEVS+1) SIGMAP INTERFACE AKI                   
!     BKI          REAL (LEVS+1) SIGMAP INTERFACE BKI                   
...
!                                                                       
!$$$                                                                    
      PARAMETER(LEVT=6,LEVZ=3,NROW=4,MYBIT=16,i100=5)
      DIMENSION IDATE(4),SI(LEVS+1),SL(LEVS),PO(KO),ZZ(KZZ)
      DIMENSION AKI(LEVS+1),BKI(LEVS+1)
      DIMENSION SIO(IGRD1,LEVS+1,JGRD1),SLO(IGRD1,LEVS,JGRD1)
      ...
      PARAMETER(NUPA=15,NUPT=6,NUZZ=3,NSUN=35,NUPAS=5)  
      ...
      DIMENSION CLON(IGRD1,JGRD1),FLON(IGRD1)
...
!     DIMENSION IL2(NUMALL)     
      INTEGER,ALLOCATABLE :: IPU(:),ITL(:),IL1(:),IL2(:)
...
!      PARAMETER(IPUOZ=34,IPUCL=223)
      PARAMETER(IPUW=40)
      PARAMETER(IPUQR=170,IPUQI=178,IPUQS=171,IPUQG=179)
      DIMENSION ipuk100(i100)
      DIMENSION IPUSUN(NSUN)                                            
      DIMENSION ITLSUN(NSUN),IL1SUN(NSUN),IL2SUN(NSUN)                  
      DIMENSION KSLP(2)                                                 
      DIMENSION IENS(5)                                                 
      DIMENSION FLX(IGRD1*JGRD1)
      DIMENSION fx100m(IGRD1,i100,JGRD1)
...
!                                                                       
     ...
     &          U10(IGRD1*JGRD1),V10(IGRD1*JGRD1),TSFC(IGRD1*JGRD1)
      COMMON/IOINDX/ KSZ,KSD,KST,KSQ,KSPSX,KSPSY,KSU,KSV,KSPS,KSZS,     &
     &               KSZSX,KSZSY,KSPN,KSTN,KSWN,KSQR,KSQI,KSQS,KSQG,    &
     &               KSOZ,KSCL,                                         &
     &               KPZ,KPU,KPV,KPR,KPQ,KPT,KPO,KPW,KPA,KPQR,KPQI,     &
     &               KPQS,KPQG,KPOZ,KPCL,                               &
     ...
     &       RLON2_s,DELX_s,DELY_s,ORTRU_s,PROJ_s
      data ipuk100/1,11,51,33,34/
      integer::JFHOUR
      !print*,'LGRIB=',30+LENPDS+LENGDS+IO*JO*(MXBIT+1)/8,JGRD1

      NUMALL=NUPA*KO+NUPT*LEVT+NUZZ*LEVZ+NSUN
      ALLOCATE(FXP(IGRD1,NUMALL,JGRD1),IPU(NUMALL),ITL(NUMALL)          &
     &        ,IL1(NUMALL),IL2(NUMALL))
...
! for KT
      ...
      KPTCL=15*KO+5*KT+1
      KZZU =15*KO+6*KT+1
      KZZV =15*KO+6*KT+KZZ+1
      KZZT =15*KO+6*KT+2*KZZ+1
      KPSUN=15*KO+6*KT+3*KZZ+1                                                 
!  SET SOME PARAMETERS                                                  
      JFHOUR=NINT(FHOUR)                                                
      ...
      KPMQI=ISRCHFLT(KO,PO,1,POT(IPUQI))-1
      KPMQS=ISRCHFLT(KO,PO,1,POT(IPUQS))-1
      KPMQG=ISRCHFLT(KO,PO,1,POT(IPUQG))-1
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      ...
      IPU(KPQI:KPQI+KPMQI-1)=IPUQI
      IPU(KPQS:KPQS+KPMQS-1)=IPUQS
      IPU(KPQG:KPQG+KPMQG-1)=IPUQG
...
!
      ...
      ITL(KPQI:KPQI+KO-1)=IPO                                             
      ITL(KPQS:KPQS+KO-1)=IPO                                             
      ITL(KPQG:KPQG+KO-1)=IPO                                             
...
!
      ...
      IL2(KPQI:KPQI+KO-1)=NPO                                             
      IL2(KPQS:KPQS+KO-1)=NPO                                             
      IL2(KPQG:KPQG+KO-1)=NPO                                             
...
!  READ SIGMA GRID DATA FOR ALL LATITUDE SLICE INTO FXS                 
        CALL RRDSGR(NSIG,IGRD1,JGRD1,NFLDS,LEVS,1,JGRD1,CLAT,CLON,SL,      &
     &           AKI,BKI,SIO,SLO,                                          &
...
!         CALL OMEGAW(IGRD1,IGRD1,LEVS,SI,SL,                            &
          CALL OMEGAW(IGRD1,IGRD1,LEVS,SIO(1,1,LAT),SLO(1,1,LAT),        &
     ...
     &               OXS,OSXS,WXS,NONHYD,AKI,BKI)
          CALL HYDRO(IGRD1,IGRD1,LEVS,SIO(1,1,LAT),SLO(1,1,LAT),         &
     &               FXS(1,KSZS,LAT),FXS(1,KST,LAT),                     &
     &               ZXS,ZXI)                                           
          CALL SIG2P(IGRD1,IGRD1,LEVS,SIO(1,1,LAT),SLO(1,1,LAT),         &
     &               FXS(1,KSPS,LAT),FXS(1,KSPN,LAT),                    &
     &               FXS(1,KSU,LAT),FXS(1,KSV,LAT),OXS,WXS,              &
     &               ZXS,ZXI,FXS(1,KSTN,LAT),RXS,FXS(1,KSQ,LAT),         &
     &               FXS(1,KSQR,LAT),FXS(1,KSQI,LAT),                    &
     &               FXS(1,KSQS,LAT),FXS(1,KSQG,LAT),                    &
     ...
     &               FXP(1,KPQ,LAT),                                     &
     &               FXP(1,KPQR,LAT),FXP(1,KPQI,LAT),                    &
     &               FXP(1,KPQS,LAT),FXP(1,KPQG,LAT),                    &
     &               FXP(1,KPCL,LAT),FXP(1,KPOZ,LAT))                    
          CALL SIG2PT(IGRD1,IGRD1,LEVS,SIO(1,1,LAT),SLO(1,1,LAT),        &
     &               FXS(1,KSPS,LAT),                                    &
     ...
     &                SIO(1,1,LAT),SLO(1,1,LAT),KT,PTKPA,                &
...
!  WRITE OUT GRIB MESSAGES SEQUENTIALLY                                 
        DO K=K1,K2                                                      
          ...
          IF(LGRIB(KAN).GT.0) THEN                                      
            ...
            !PRINT *,' GRIB2 WRITTEN TO ',NPGB,' OF LENGTH ',LGRIB(KAN)  
            IF(ITL(K).EQ.ITLSLR) THEN                                   
              CALL CTLVAR(NSLR,IPU(K),ITL(K),IPTV)
            ELSE                                                        
              CALL CTLVAR(NPRS,IPU(K),ITL(K),IPTV)
            ENDIF                                                       
          ENDIF                                                         
        ENDDO                                                           
      ENDDO                                                             
...
!yj     ELSEIF(IPUK.EQ.170.OR.IPUK.EQ.171) THEN                         
!yj       WRITE(*,'("...CHANGED FIELD 170 AND 171 TO 134 AND 135")')    
!yj       IPUK=IPUK-46                                                  
...
! SAVE FOR LATER                                                        
        ...
        IF(IPUK.EQ.1.AND.ITLK.EQ.1) THEN                                
          IPUK=0                                                        
          ICHECK=ICHECK+1                                               
          print*,'icheck=',icheck,'var=ps'
          DO I=1,KF                                                     
            PS(I)=FLX(I)                                                
          ENDDO                                                         
        ENDIF                                                           
        IF(IPUK.EQ.11.AND.ITLK.EQ.1) THEN
          ICHECK=ICHECK+1
          print*,'icheck=',icheck,'var=ts'
          DO I=1,KF
            TSFC(I)=FLX(I)
          ENDDO
        ENDIF
        IF(IPUK.EQ.33.AND.ITLK.EQ.105) THEN
          ICHECK=ICHECK+1
          print*,'icheck=',icheck,'var=u10'
          DO I=1,KF
            U10(I)=FLX(I)
          ENDDO
        ENDIF
        IF(IPUK.EQ.34.AND.ITLK.EQ.105) THEN
          ICHECK=ICHECK+1
          print*,'icheck=',icheck,'var=v10'
          DO I=1,KF
            V10(I)=FLX(I)
          ENDDO
        ENDIF
...
! change from cb to pa (1 centbar=1000 pa)
                plry(kk  )=fxs(i,ksps,j)*slo(i,k,j)*1000.  
                plvl(kk+1)=fxs(i,ksps,j)*sio(i,k,j)*1000.
              enddo
              plvl(1)=fxs(i,ksps,j)*sio(i,levs+1,j)*1000.
...
! use new landsfc
          ...
          ENDIF                                                         
!yj:print          
!         IF(IPUK.EQ.11.OR.IPUK.EQ.51) then
!         print*,'gribit, when ipuk=',ipuk
!         print*,'IPUK,ITLK,IL1K,IL2K',IPUK,ITLK,IL1K,IL2K
!         ENDIF
!yj:print          
     ...
     &                GRIB(1,1),LGRIB(1),IERR)                          
          IF(LGRIB(1).GT.0) THEN                                        
            ...
            IF(ITLK.EQ.ITLSLR) THEN                                     
              CALL CTLVAR(NSLR,IPUK,ITLK,IPTV)
            ELSEIF(ITLK.EQ.ITLDLR) THEN                                 
              CALL CTLVAR(NDLR,IPUK,ITLK,IPTV)
            ELSE                                                        
              CALL CTLVAR(NPRS,IPUK,ITLK,IPTV)
            ENDIF                                                       
          ENDIF                                                         
        ENDIF                                                           
        GO TO 911                                                       
!                                                                       
910     continue !PRINT *,' ERROR READING FLX RECORD '                            
911     continue !PRINT *,' READ NEXT RECORD.'                                    
      ENDDO                                                             
...
! IF NEED TRANSFORM FOM ONE GRID TO ANOTHER                             
     ...
     &                   N00,N10,N11,N01,D00,D10,D11,D01)               
          LBMO=.TRUE.
        ENDIF                                                           
        !print*,'YJ:IBMS=',IBMS,'it should be 0'
        !print*,'YJ:IPUK,ITLK,IL1K,IL2K:',IPUK,ITLK,IL1K,IL2K
        CALL GRIBIT(AWIPS,LBMO,IDRT,IGRID,IO,JO,MXBIT,0.0,                &    
     &            LNPDS,IPTV,ICEN,IGEN,IBMS,                             &
     &            IPUK,ITLK,IL1K,IL2K,                                   &
     &            IYR,IMO,IDY,IHR,IFTU,IP1,IP2,ITR,                      &
     ...
     &            GRIB(1,1),LGRIB(1),IERR)                              
        ...
        ENDIF                                                           
      ENDIF                                                             
!yj:----------dew point temperature at 2m------------->>
      IF( ICHECK.EQ.6 ) THEN                                            
      PRINT *,' OUTPUT DewT2m='
        DO I=1,KF                                                       
          P2=PS(I)*EXP(-GOR2/T2(I))*1.E-3                               
          pv=P2*Q2(I)/(EPS-EPSM1*Q2(I))
...
! correct a bug
     ...
     &            GRIB(1,1),LGRIB(1),IERR)                              
      ...
      ENDIF                                                             
!                                                                       
!xb118:-----P T Q U V at 2m (fx100m(igrd1,5,jgrd1)---->>
      PRINT *,' OUTPUT fx100m:'
      call out100(igrd1,jgrd1,levs,T2(:),Q2(:),U10(:),V10(:)            &
     &       ,FXS(:,ksps,:),TSFC,FXS(:,kspn:(kspn+levs-1),:)            &
     &       ,FXS(:,ksu:(ksu+levs-1),:),FXS(:,ksv:(ksv+levs-1),:)       &
     &       ,FXS(:,kstn:(kstn+levs-1),:),FXS(:,ksq:(ksq+levs-1),:)     &
     &       ,fx100m)

              print*,fx100m(igrd1/2,1:5,jgrd1/2)
        LBM=.TRUE.                                                 
        IPTV=2
        ITLK=105                                                        
        IL1K=0                                                          
...
! IF NEED TRANSFORM FOM ONE GRID TO ANOTHER                             
        IF( JCHECK.EQ.0 ) THEN                                          
          DO J=1,JO                                                     
            DO I=1,IO                                                   
              AWIPS(I,J)=fx100m(I,n,J)
            ENDDO                                                       
          ENDDO                                                         
          LBMO=LBM
        ELSE                                                            
          CALL TOAWIPS(fx100m(1,n,1),IGRD1*JGRD1,AWIPS,IO*JO,           &
     &                   N00,N10,N11,N01,D00,D10,D11,D01)               
          LBMO=.TRUE.
        ENDIF                                                           
        CALL GRIBIT(AWIPS,LBMO,IDRT,IGRID,IO,JO,MXBIT,0.0,              &
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      ...
      END                                                               
!-----------------------------------------------------------------------
      SUBROUTINE RRDSGH(NSIG,FHOUR,IDATE,SI,SL,AKI,BKI,                  &
     &                  IWAV1,JWAV1,IGRD1,JGRD1,LEVS,NONHYD,NFLDS,       &
     &                  PROJ,TRUTH,ORIENT,CENLAT,CENLON,                 &
     &                  GRDLEFT,GRDBOTM,DELX,DELY,IRET,NCLD)               
!$$$  SUBPROGRAM DOCUMENTATION BLOCK                                    
...
!                                                                       
! USAGE:     CALL RRDSGH(NSIG,FHOUR,IDATE,SI,SL,AKI,BKI,                
...
! aki, bki for sigmap coordinate
      ALLOCATE(AKI4(LEVS+1),BKI4(LEVS+1))
      READ(NSIG,END=91,ERR=92) AKI4,BKI4
      AKI(1:LEVS+1)=AKI4(1:LEVS+1)
      BKI(1:LEVS+1)=BKI4(1:LEVS+1)
      DEALLOCATE(AKI4,BKI4)

      print*,'check aki =',aki(1:levs+1)
!                                                                       
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      END                                                               
!-----------------------------------------------------------------------
      SUBROUTINE RRDSGR(NSIG,IGRD1,JGRD1,NFLDS,LEVS,LAT1,LAT2,CLAT,        &
     &                  CLON,SL,AKI,BKI,SIO,SLO,                           &
     &                  NONHYD,RDELX2,RDELY2,RLAT1,RLON1,RLAT2,RLON2,      &
     &                  XMAP,NTRAC,NCLD,F)        
!$$$  SUBPROGRAM DOCUMENTATION BLOCK                                    
!                                                                       
...
!$$$                                                                    
      ...
      DIMENSION CLON(IGRD1*JGRD1)
      DIMENSION AKI(LEVS+1),BKI(LEVS+1)
      DIMENSION SIO(IGRD1,LEVS+1,LAT2-LAT1+1)
      DIMENSION SLO(IGRD1,LEVS,LAT2-LAT1+1)
      COMMON/IOINDX/ KSZ,KSD,KST,KSQ,KSPSX,KSPSY,KSU,KSV,KSPS,KSZS,       &
     &               KSZSX,KSZSY,KSPN,KSTN,KSWN,KSQR,KSQI,KSQS,KSQG,      &
     &               KSOZ,KSCL,                                           &
     &               KPZ,KPU,KPV,KPR,KPQ,KPT,KPO,KPW,KPA,KPQR,KPQI,       &
...
! LNPS                                                                  
      ...
      DO LAT=LAT1X,LAT2X                                                
        ...
        ENDDO                                                           
      ENDDO                                                             
! Caculate SL value for sigmap coordi.
      DO LAT=LAT1,LAT2
        LAN=LAT-LAT1+1
        JLAT=(LAT-1)*IGRD1
      DO I=1,IGRD1                                                    
      DO K=1,LEVS+1
      SIO(I,K,LAN)=(AKI(K)+BKI(K)*F(I,KSPS,LAN)*10.)/(F(I,KSPS,LAN)*10.)
      ENDDO
      ENDDO
      ENDDO
      print*,'check sio and ps=',SIO(1,1:levs+1,1),F(1,KSPS,1)
      DO LAT=LAT1,LAT2
        LAN=LAT-LAT1+1
        JLAT=(LAT-1)*IGRD1
       DO I=1,IGRD1                                                    
         DO K=1,LEVS
          SLO(I,K,LAN)=(SIO(I,K,LAN)+SIO(I,K+1,LAN))/2.
         ENDDO
       ENDDO
       ENDDO
! T                                                                     
...
!cc
      IF( NCLD .EQ. 2 ) THEN            ! WSM3
        DO K=1,LEVS
          ...
          PRINT *,' READ  QR+QS  AT LEV= ',K,GR1(1)                           
          DO LAT=LAT1,LAT2
            LAN=LAT-LAT1+1
            JLAT=(LAT-1)*IGRD1
            DO I=1,IGRD1
      ...
      IF( NCLD .EQ. 4 ) THEN
        ...
        DO K=1,LEVS
          ...
          PRINT *,' READ  QI  AT LEV= ',K,GR1(1)                           
          DO LAT=LAT1,LAT2
            LAN=LAT-LAT1+1
            JLAT=(LAT-1)*IGRD1
            DO I=1,IGRD1
              F(I,KQI,LAN)=GR1(I+JLAT)
              F(I,KKK,LAN)=F(I,KKK,LAN)+GR1(I+JLAT)
            ENDDO
          ENDDO
        ENDDO
        DO K=1,LEVS
          ...
          PRINT *,' READ  QS  AT LEV= ',K,GR1(1)                           
          DO LAT=LAT1,LAT2
            LAN=LAT-LAT1+1
            JLAT=(LAT-1)*IGRD1
            DO I=1,IGRD1
              F(I,KQS,LAN)=GR1(I+JLAT)
              F(I,KKK,LAN)=F(I,KKK,LAN)+GR1(I+JLAT)
            ENDDO
          ENDDO
        ENDDO
      ...
      IF( NCLD .EQ. 5 ) THEN
        ...
        DO K=1,LEVS
          ...
          PRINT *,' READ  QG  AT LEV= ',K,GR1(1)                           
          DO LAT=LAT1,LAT2
            LAN=LAT-LAT1+1
            JLAT=(LAT-1)*IGRD1
            DO I=1,IGRD1
              F(I,KQG,LAN)=GR1(I+JLAT)
              F(I,KKK,LAN)=F(I,KKK,LAN)+GR1(I+JLAT)
            ENDDO
          ENDDO
        ENDDO
...
! PN
      ...
      DO LAT=LAT1,LAT2
        ...
        JLAT=(LAT-1)*IGRD1
        DO I=1,IGRD1
          F(I,KSPS,LAN)=F(I,KSPN,LAN)/SLO(I,1,LAN)
...
! hydro P
      DO K=1,LEVS
        ...
        DO LAT=LAT1,LAT2
          ...
          JLAT=(LAT-1)*IGRD1
          DO I=1,IGRD1
            F(I,KPN,LAN)=F(I,KSPS,LAN)*SLO(I,K,LAN)
          ENDDO
        ENDDO
      ENDDO
! hydro T
...
!-----------------------------------------------------------------------
!FPP$ NOCONCUR R                                                        
      SUBROUTINE SIG2P(IM,IX,KM,SIO,SLO,PS,PP,                             &
     &                 US,VS,OS,WS,ZS,ZI,TS,RS,QS,QRS,QIS,QSNS,QGS,        &
     &                 CLS,OZS,                                            &
     &                 KO,PO,UP,VP,OP,WP,ZP,TP,RP,QP,QRP,QIP,QSNP,QGP,     &
     &                 CLP,OZP)
!$$$  SUBPROGRAM DOCUMENTATION BLOCK                                    
!                                                                       
...
!     QRS      - REAL (IX,KM) CLOUD RAIN WAGER MIXING RATIO IN KG/KG
!     QIS      - REAL (IX,KM) CLOUD ICE MIXING RATIO IN KG/KG
!     QSNS     - REAL (IX,KM) SNOW MIXING RATIO IN KG/KG
!     QGS      - REAL (IX,KM) GRAUPEL MIXING RATIO IN KG/KG
...
!     QRP      - REAL (IX,KO) CLOUD RAIN WAGER MIXING RATIO IN KG/KG
!     QIP      - REAL (IX,KO) CLOUD ICE MIXING RATIO IN KG/KG
!     QSNP     - REAL (IX,KO) SNOW MIXING RATIO IN KG/KG
!     QGP      - REAL (IX,KO) GRAUPEL MIXING RATIO IN KG/KG
...
!              ZP(I,K)=WU*ZS(I,KU)+WD*ZS(I,KD)
              ...
              QIP(I,K)=WU*QIS(I,KU)+WD*QIS(I,KD)
              QSNP(I,K)=WU*QSNS(I,KU)+WD*QSNS(I,KD)
              QGP(I,K)=WU*QGS(I,KU)+WD*QGS(I,KD)
              KI=KD+1
              DI=ASI(I,KI)-(ASK-APS(I))
              KL=NINT(KI-0.5+SIGN(0.5,DI))
              WL=DI/(ASI(I,KI)-ASL(I,KL))
              WI=1.-WL
...
!  INTERPOLATE SIGMA TO PRESSURE OUTSIDE THE MODEL DOMAIN               
        DO I=1,IM                                                       
!yj          ASK=APO(K)-LOG(PP(I,K))+ASL(I,K)
          ASK=APO(K)
...
!yj          
            ...
            WP(I,K)=WS(I,1)
!yj            WL=ASK/ASL(I,1)                                               
            WL=(ASK-APS(I))/ASL(I,1)    
            WI=1.-WL                                                    
            ZP(I,K)=WI*ZI(I,1)+WL*ZS(I,1)                               
!yj            WU=(ASL(I,1)-ASK)/(ASL(I,1)-ASL(I,2))                             
            WU=(ASL(I,1)-(ASK-APS(I)))/(ASL(I,1)-ASL(I,2))  
            WD=1.-WU                                                    
...
!yj          ELSEIF(ASK.LE.ASL(I,KM)) THEN                                   
...
!yj          
            ...
            ZP(I,K)=ZS(I,KM)+ROG*TVKM*(ASL(I,KM)-(ASK-APS(I)))
            TP(I,K)=TS(I,KM)                                            
            RP(I,K)=RS(I,KM)                                            
            QP(I,K)=QS(I,KM)                                            
            QRP(I,K)=QRS(I,KM)
...
!-----------------------------------------------------------------------
!FPP$ NOCONCUR R                                                        
      SUBROUTINE SIG2PT(IM,IX,KM,SIO,SLO,                                 &
     &                  PS,US,VS,TS,QS,QSS,                               &
...
!                                                                       
!$$$                                                                    
      DIMENSION SIO(IX,KM+1),SLO(IX,KM),PS(IM)                                  
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      DO K=1,KT                                                         
        ...
        DO KS=1,KM                                                      
          DO I=1,IM                                                   
          IF((K-(1-SIO(I,KS))*RMIN.GT.0.).AND.                           &
     &       (K-(1-SIO(I,KS+1))*RMAX.LT.1.))THEN
              R=PS(I)/PT                                                
              FKS=MIN(K-(1-SIO(I,KS))*R,1.)-MAX(K-(1-SIO(I,KS+1))*R,0.)       
              IF(FKS.GT.0.) THEN                                        
...
!-----------------------------------------------------------------------
!FPP$ NOCONCUR R                                                        
      SUBROUTINE SIG2TP(IM,IX,KM,CLAT,SIO,SLO,                              &
     &                  P,U,V,T,ZL,ZI,                                    &
...
!  CALCULATE POINT NUMBER IF NECESSARY                                  
      DO I=1,L                                                          
        IF(P(I).LE.0.) THEN                                             
          K=1                                                           
          DO WHILE(K.LE.N.AND.XP(I).GT.X(K))                             
            K=K+1                                                       
          ENDDO                                                         
          P(I)=K-0.5                                                    
        ENDIF                                                           
...
!-----------------------------------------------------------------------
!FPP$ NOCONCUR R                                                        
      SUBROUTINE SUNDRY(IM,IX,KM,KSLP,KLI,K5Z,CLAT,SIO,SLO,KT,PT,         &
...
!  SUNDRY TROPOPAUSE FIELDS                                             
      CALL SIG2TP(IM,IX,KM,CLAT,SIO,SLO,P,U,V,T,ZL,ZI,                   &
...
!  LIFTED INDEX                                                         
      IF(KLI.GT.0) THEN                                                 
        CALL LIFTIX(IM,IX,KT,PT,KM,SIO,SLO,PS,TM(1,KLI),TPT,QPT,T,Q,     &
...
!  AVERAGE RELATIVE HUMIDITY 1                                          
      ...
      DO K=1,KM                                                         
        DO I=1,IM                                                     
        DS=MIN(SIO(I,K),SBRH1)-MAX(SIO(I,K+1),STRH1)                          
        IF(DS.GT.0.) THEN                                               
...
!  AVERAGE RELATIVE HUMIDITY 2                                          
      ...
      DO K=1,KM                                                         
        DO I=1,IM                                                     
        DS=MIN(SIO(I,K),SBRH2)-MAX(SIO(I,K+1),STRH2)                          
        IF(DS.GT.0.) THEN                                               
...
!  AVERAGE RELATIVE HUMIDITY 3                                          
      ...
      DO K=1,KM                                                         
        DO I=1,IM                                                     
        DS=MIN(SIO(I,K),SBRH3)-MAX(SIO(I,K+1),STRH3)                          
        IF(DS.GT.0.) THEN                                               
...
!  BOTTOM SIGMA FIELDS INTERPOLATED FROM FIRST TWO MODEL LAYERS         
      SL1K=(SL1*1.E-2)**(-ROCP)                                         
      DO I=1,IM                                                         
        F2=LOG(SLO(I,1)/SL1)/LOG(SLO(I,1)/SLO(I,2))                                
...
!                                                                       
      ELSE IF( NPROJ.EQ.4 ) THEN
        ...
        IF(CENLAT.LT.0.E0) CENLAT = TWOPI + CENLAT
        A2 =  RERTH * COS( TRUTH )
        X=A2*(BLON-CENLON)                                              
        Y=A2*(BLAT-CENLAT)                                              
!
...
!                                                                       
      ELSE IF( NPROJ.EQ.4 ) THEN
        ...
        BLAT = Y / A2 + CENLAT                                          
        BLAT = MOD(BLAT,TWOPI)                                          
        IF(BLAT.LT.0.E0) BLAT = TWOPI + BLAT                          
!
      ENDIF
!                                                                       
! OUTPUT IN RADIANS                                                     
...
!                  IPTV=131 IS ADDED FOR WVUFLX, WVUFLX - 2021.10.07
!                  IPTV=133 IS ADDED FOR MAX,MIN SPFH --- 2021.10.07
!   OUTPUT ARGUMENTS:
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      IF(IPTV.EQ.1.OR.IPTV.EQ.2) THEN
        ...
        IDS(170)=9      ! RAIN WATER MIXING RATIO (KG/KG)
        IDS(171)=9      ! SNOW MIXING RATIO (KG/KG)
        ...
        IDS(178)=9      ! ICE MIXING RATIO (KG/KG)
        IDS(179)=9      ! GRAUPEL MIXING RATIO (KG/KG)
        IDS(180)=1      ! WIND GUST (M/S)
        ...
        IDS(184)=5      ! Y-GRADIENT HEIGHT (M/M)
                        !
                        !
        IDS(187)=3      ! LIGHTNING (FLASHS/KM2/PER_OUTPUT_TIME_PERIOD)
                        !
                        !
                        !
                        !
        ...
        IDS(254)=7      ! RICHARDSON NUMBER ()
                        !
      ELSEIF(IPTV.EQ.131) THEN
        IDS(242)=0      ! WATER VAPOR ZONAL FLUX (VERTICAL INT) (KG/M)
        IDS(243)=0      ! WATER VAPOR MERIDIONAL FLUX (VERTICAL INT) (KG/M)
                        !
      ELSEIF(IPTV.EQ.133) THEN
        IDS(204)=5      ! MAXIMUM SPECIFIC HUMIDITY (KG/KG)
        IDS(205)=5      ! MINIMUM SPECIFIC HUMIDITY (KG/KG)
      ENDIF
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      RETURN
      END
...
333   CONTINUE
     ...
     &                  RLAT,RLON,IZ)                                        
!$$$                                                                    
      DIMENSION RLAT(JM),IZ(KM),RLON(IM)                                 
...
!       WRITE(NN,108) IM,STRLON,DLON                                    
 ...
 1081 FORMAT('xdef',I5,' levels')
 1082 FORMAT(10(F10.4,1X))                                                   
...
!      WRITE(NN,114) HOUR,DAY,MON(IMON),IYR,IFH                          
      WRITE(NN,114) HOUR,DAY,MON(IMON),IYR
 112  FORMAT('zdef',I5,' levels')                                       
 113  FORMAT(10I8)                                                      
! 114  FORMAT('tdef 99 linear ',A2,'Z',A2,A3,I4,I10,'hr')                
 114  FORMAT('tdef 99 linear ',A2,'Z',A2,A3,I4,' 6hr')
      WRITE(NN,115)                                                     
...
!                                                                       
 ...
 100  FORMAT('endvars')                                                 
      RETURN                                                            
      END                                                               
      SUBROUTINE CTLVAR(NN,NUMVAR,NUMSFC,IPTV)                               
      CHARACTER VARSFC*8,VARWRT*8                                       
      CHARACTER VAR*5,SFC*3,DSL*50 
      CHARACTER VAR131*5,SFC131*3,DSL131*50 
      CHARACTER VAR133*5,SFC133*3,DSL133*50 
      COMMON /COMCTL/ VAR(255),SFC(255),DSL(255),                        &
     &                VAR131(255),SFC131(255),DSL131(255),               &
     &                VAR133(255),SFC133(255),DSL133(255)                        
      SAVE NUMVARS,NUMSFCS,IPTVS,NUMS,NNS                                     
      DATA NUMVARS,NUMSFCS,IPTVS,NUMS,NNS/0,0,0,0,0/ 
!                                                                       
      IF( NUMVAR.NE.NUMVARS .OR. NUMSFC.NE.NUMSFCS ) THEN               
        IF(NUMVARS.NE.0) THEN                                           
          IF((IPTVS.EQ.0).OR.(IPTVS.EQ.1).OR.(IPTVS.EQ.2)) THEN
            VARSFC=VAR(NUMVARS)//SFC(NUMSFCS) 
          ELSEIF(IPTVS.EQ.131) THEN
            VARSFC=VAR131(NUMVARS)//SFC131(NUMSFCS)
          ELSEIF(IPTVS.EQ.133) THEN
            VARSFC=VAR133(NUMVARS)//SFC133(NUMSFCS)
          ELSE
            PRINT*,'ERROR PARAMETER TABLE VERSION: APPENDING'
          ENDIF
          VARWRT='        '                                             
          M=0                                                           
          DO N=1,8                                                      
          ...
          IF(NUMS.EQ.1) NUMS=0                                          
          IF((IPTVS.EQ.0).OR.(IPTVS.EQ.1).OR.(IPTVS.EQ.2)) THEN
            IF(((NUMVARS.EQ.33).OR.(NUMVARS.EQ.34).OR.(NUMVARS.EQ.180))    &
     &         .AND.(NUMSFCS.EQ.105) ) THEN
            WRITE(NNS,202) VARWRT,                                         &
     &                  NUMS,NUMVARS,NUMSFCS,DSL(NUMVARS)
            ELSEIF(((NUMVARS.EQ.11).OR.(NUMVARS.EQ.51).OR.                 &
     &              (NUMVARS.EQ.15).OR.(NUMVARS.EQ.16).OR.                 &
     &              (NUMVARS.EQ.52).OR.(NUMVARS.EQ.17)).AND.               &
     &              (NUMSFCS.EQ.105) ) THEN
            WRITE(NNS,201) VARWRT,                                         &
     &                  NUMS,NUMVARS,NUMSFCS,DSL(NUMVARS)
            ELSE
     ...
     &                  NUMS,NUMVARS,NUMSFCS,DSL(NUMVARS)
            ENDIF
          ELSEIF(IPTVS.EQ.133) THEN
            IF(((NUMVARS.EQ.204).OR.(NUMVARS.EQ.205))                      &
     &         .AND.(NUMSFCS.EQ.105) ) THEN
            WRITE(NNS,201) VARWRT,                                         &
     &                  NUMS,NUMVARS,NUMSFCS,DSL133(NUMVARS)
            ENDIF
          ELSEIF(IPTVS.EQ.131) THEN
            WRITE(NNS,200) VARWRT,                                         &
     &                  NUMS,NUMVARS,NUMSFCS,DSL131(NUMVARS)
          ELSE
            PRINT*,'ERROR PARAMETER TABLE VERSION: WRITING'
            PRINT*,'IPTVS=',IPTVS,'VARWRT=',VARWRT
            STOP 88
          ENDIF
200       FORMAT(A8,I8,I4,',',I3,',0 ',A50)
201       FORMAT(A8,I8,I4,',',I3,',2 ',A50)
202       FORMAT(A8,I8,I4,',',I3,',10 ',A50)
        ENDIF                                                           
        NUMS=1                                                          
      ELSE                                                              
        NUMS=NUMS+1                                                     
...
!               !12345678901234567890123456789012345678901234567890     
      DO I=1,255
       ...
       DSL131(I)='XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
       DSL133(I)='XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
      ENDDO
       DSL(001)='PRESSURE (PA)                                     '    
       ...
       DSL(170)='RAIN WATER MIXING RATIO (KG/KG)                   '
       DSL(171)='SNOW MIXING RATIO (KG/KG)                         '
       ...
       DSL(178)='ICE MIXING RATIO (KG/KG)                          '
       DSL(179)='GRAUPEL MIXING RATIO (KG/KG)                      '
       DSL(180)='MAXIMUM WIND GUSTS (M/S)                          '    
       ...
       DSL(254)='RICHARDSON NUMBER ()                              '    
		!                                                  '                  
       DSL131(242)='WATER VAPOR ZONAL FLUX (VERTICAL INT) (KG/M)   '
       DSL131(243)='WATER VAPOR MERIDIONAL FLUX (VERTICAL INT) (KG/M)'
		!                                                  '                  
       DSL133(204)='MAXIMUM SPECFIC HUMIDITY(KG/KG)                '
       DSL133(205)='MINIMUM SPECFIC HUMIDITY(KG/KG)                '
		!                                                  '                  
      RETURN                                                            
      END                                                               
