...
!lzl      implicit none
!
      PROGRAM mtnlm7_slm30g_rsm_variant
...
!     HJ for T1000 test. Must set to 1 for now.
      ...
      MTNRES = 1
      IM = IM + 1 ! igrd1 = igrd + 1
      JM = JM + 1 ! jgrd1 = jgrd + 1
      NM = IM / 2 ! Hard-coded
      NR = 0      ! Ditto
      NF0 = 0     ! No-op, useless
      NF1 = 0     ! Ditto
      EFAC = 0.0  ! Do not scale orography
      BLAT = 0.0  ! Do not reverse lat-lon
      print*,'lzl print  MTNRES,IM,JM,NM,NR,NF0,NF1,EFAC,BLAT'
      ...
      print*,'lzl before TERSUB=
     &  IMN,JMN,IM,JM,NM,NR,NF0,NF1,NW,EFAC,BLAT'
...
!
      ...
      integer, parameter :: spkind = selected_real_kind(6, 37)
      logical, parameter :: check_nans=.false.
!     logical, parameter :: check_nans=.true.
!
      integer IMN,JMN,IM,JM,NM,NR,NF0,NF1,NW,mlmax,NFLT
      real    efac, blat
...
!
     ...
     &                     hprime(:,:,:)
      ...
      LOGICAL SPECTR, REVLAT, FILTER, FFTW
...
!
      ...
      pi     = 4.0 * atan(1.0)
      DEGRAD = 180./PI
      SPECTR = .FALSE.       ! Forbid using gaussian grid and force regular lat-lon grid
      FILTER = NM > 0        ! Spectral filter
      FFTW   = .TRUE.        ! Force to use FFTW
      mlmax  = (NM*(NM+1))/2
      NFLT   = 0             ! Power of Lanczos filter for topography
...
!     MSKSRC = 1
      ...
      REVLAT = .FALSE.       ! Do not reverse lat-lon
...
!    &", recl=43200*21600, access='direct',iostat=istat)
     ...
     &                                    access='direct',iostat=istat)
        ...
        ELSE 
!
      print*,'lzl open landcover30.fixed '
...
! --------------
     ...
     &,                 MSKSRC
        ...
        ELSE
          print *,'lzl UMD lake, UMD(500,1500)=',UMD(500,1500),MSKSRC
          print *,' UMD lake, UMD(500,500)=',UMD(500,500),MSKSRC
        ENDIF
          print*,'lzl good UMD land cover read and MSKSRC = 1'
! --------------
! ---      good UMD land cover read and MSKSRC = 1
      ELSE
       print*,'UMD is dependent on topography'
      ENDIF        
!
...
! --- transpose glob as USGS 30" is from dateline and NCEP std is 0
      ...
      print *,' lzl After read_g, glob(500,1500:1600)='
      print*,glob(500,1500:1600)
!
...
! ---
      CASE(-99)
       print*,'**** ZSLM/UMD is defined from topo'
         do j=1,jmn
          do i=1,imn
            if(glob(i,j).le.0)zslm(i,j)=0
            zavg(i,j)=glob(i,j)
          enddo
         enddo 
      END SELECT
...
!     Forbid using gaussian grid and force regular lat-lon grid
      numi = im
!
...
!
      ...
      do i = 1, imn
        glon(i) = 0.0 + real(i - 1) * delxn + 0.5 * delxn
      end do
...
!     & ' After GICE ZAVG(1,112)=',ZAVG(1,112),ZSLM(1,112)
      ...
      ! -pi/2 <= flat <= pi/2
      !     0 <= flon <= 2*pi
      call rsm_setlatlon(flat, flon, im, jm)

      ! Convert radian to degree
      flat = flat * degrad
      flon = flon * degrad

      flat2d = reshape(flat, (/ im, jm /))
      flon2d = reshape(flon, (/ im, jm /))
...
!
      CALL MAKEMT(ZAVG,ZSLM,ORO,SLM,VAR,VAR4,GLAT,glon,
     &            IST,IEN,JST,JEN,IM,JM,IMN,JMN,numi,
     &            flat2d,flon2d)
...
!
     ...
     1            IST,IEN,JST,JEN,IM,JM,IMN,JMN,numi)
...
!
      call minmxj(IM,JM,ORO,'     ORO')
      CALL MAKEOA(ZAVG,VAR,OA,OL,IWORK,ELVMAX,ORO,
...
!
      oro_s = oro
!
      write(0,*)' NF1, NF0, FILTER, FFTW=',NF1,NF0,FILTER,FFTW

      IF (FILTER) THEN      !  SPECTRALLY TRUNCATE AND FILTER OROGRAPHY
        IF ( FFTW ) THEN
          write(0,*) 'use FFTW for spectral transform'

          if (mod(im, 2) == 0) then
            ! im is even: OK
            call gfstopo(im,jm,nm,mlmax,nflt,oro,orf)
          else
            ! im is odd: Pad with zero
            oropad = 0.0
            oropad(1:im, 1:jm) = oro

            call gfstopo(im+1,jm+1,nm,mlmax,nflt,oropad,orfpad)

            orf = orfpad(1:im, 1:jm)
          end if
        ENDIF
      ELSE
        IF (REVLAT) THEN
...
!     OUTPUT BINARY FIELDS
      ...
      print *, 'Writing output rsm-lat.bin'
      open(unit=41, action='write', file='rsm-lat.bin',
     &     form='unformatted', status='replace')
      write(unit=41) real(flat2d, spkind)
      close(unit=41)

      print *, 'Writing output rsm-lon.bin'
      open(unit=42, action='write', file='rsm-lon.bin',
     &     form='unformatted', status='replace')
      write(unit=42) real(flon2d, spkind)
      close(unit=42)

      print *, 'Writing output rsm-oro.bin'
      open(unit=51, action='write', file='rsm-oro.bin',
     &     form='unformatted', status='replace')
      write(unit=51) real(   oro, spkind)
      close(unit=51)

      print *, 'Writing output rsm-oro-smoothed.bin'
      open(unit=52, action='write', file='rsm-oro-smoothed.bin',
     &     form='unformatted', status='replace')
      write(unit=52) real(   orf, spkind)
      close(unit=52)

      print *, 'Writing output rsm-lsm.bin'
      open(unit=53, action='write', file='rsm-lsm.bin',
     &     form='unformatted', status='replace')
      write(unit=53) real(   slm, spkind)
      close(unit=53)

      print *, 'Writing output rsm-hprime.bin'
      open(unit=54, action='write', file='rsm-hprime.bin',
     &     form='unformatted', status='replace')
      write(unit=54) real(hprime, spkind)
      close(unit=54)

      ! RSM does not need grib files of orographic variables
      ! Skip the rest and stop here

      print *, 'Normal termination'
      END
      SUBROUTINE MAKEMT(ZAVG,ZSLM,ORO,SLM,VAR,VAR4,
     &                  GLAT,glon,IST,IEN,JST,JEN,IM,JM,IMN,JMN,numi,
     &                  flat2d,flon2d)
!
!lzl      implicit none
!
     ...
     &,       GLAT(JMN),glon(imn)
      real :: flat2d(im, jm), flon2d(im, jm)
!
      integer i, j, im1, jm1, ii1, i1, j1
      real    xnsum, xland, xwatr, xl1, xs1
     &,       xw1, xw2, xw4, height
!
...
!  (*j*)  for hard wired zero offset (lambda s =0) for terr05 
      ...
      print *, 'Compute ist ien jst jen for regional domain'
      ...
      DO J=1,JM
        DO I=1,numi(j)
          ...
          do i1 = 1, imn
            if (i > 1 .and. i < numi(j)) then ! Interior
              if (glon(i1) >= 0.5 * (flon2d(i - 1, j) +
     &                               flon2d(i, j)) .and.
     &            glon(i1) <  0.5 * (flon2d(i, j) +
     &                               flon2d(i + 1, j))) then
                ist(i, j) = i1
                ien(i - 1, j) = i1 - 1
                exit
     ...
     &                               flon2d(i - 1, j))) then
                ...
                ien(i - 1, j) = i1 - 1
                ien(i, j) =
     &            min(ist(i, j) + ien(i - 1, j) - ist(i - 1, j),
     &                imn)
                exit
              end if
            end if
        ...
        do j1 = 1, jmn
          if (j > 1 .and. j < jm) then ! Interior
            if (glat(j1) >= 0.5 * (flat2d(i, j - 1) +
     &                             flat2d(i, j)) .and.
     &          glat(j1) <  0.5 * (flat2d(i, j) +
     &                             flat2d(i, j + 1))) then
              jst(j) = j1
              jen(j - 1) = j1 - 1
              exit
     ...
     &              jmn)
      ...
      if (any(ist < 0)) then
        print *, 'Illegal ist value'
        stop
      end if

      if (any(ien < 0) .or. any(ien - ist < 0)) then
        print *, 'Illegal ien value'
        stop
      end if

      if (any(jst < 0)) then
        print *, 'Illegal jst value'
        stop
      end if

      if (any(jen < 0) .or. any(jen - jst < 0)) then
        print *, 'Illegal jen value'
        stop
      end if
...
!
     ...
     1           GLAT,IST,IEN,JST,JEN,IM,JM,IMN,JMN,numi)
!
...
!
      ...
      SUBROUTINE MAKEOA(ZAVG,VAR,OA4,OL,IOA4,ELVMAX,
     1           ORO,oro1,XNSUM,XNSUM1,XNSUM2,XNSUM3,XNSUM4,
     2           IST,IEN,JST,JEN,IM,JM,IMN,JMN,numi)
!
!lzl      implicit none
!
      integer IM,JM,IMN,JMN
...
!
      DO J=1,JM-2
        DO I=1,numi(j)
        II = I + 1
        ! Should not wrap around for regional domain
        IF (II .GT. numi(j)) cycle
...
!
      DO KWD = 1, 4
        ...
        oa4(1,  :, kwd) = oa4(2,      :, kwd)
         ...
         ol(:,  1, kwd) =  ol(:,      2, kwd)
         ol(:, jm, kwd) =  ol(:, jm - 1, kwd)
        oa4(:,  1, kwd) = oa4(:,      2, kwd)
        oa4(:, jm, kwd) = oa4(:, jm - 1, kwd)
      ENDDO
!
...
!!mr
      ...
      integer*2 ia(len)
      character*7 tile
      integer iaamax, iaamin, j, m, ja, kount, tmp
...
!
      mean = isum/kount
      tmp  = (sum2/(kount))-mean**2
!      std  = ifix(sqrt(float((sum2/(kount))-mean**2)))
      std  = ifix(sqrt(float(tmp)))
!
...
C-----------------------------------------------------------------------
      subroutine rsm_setlatlon(flat, flon, igrd1, jgrd1)

      integer, intent(in) :: igrd1, jgrd1
      real, intent(out) :: flat(igrd1 * jgrd1), flon(igrd1 * jgrd1)
     ...
     &        rcenlat, rcenlon, rlftgrd, rbtmgrd
      ...
      real(dpkind), parameter :: pi = atan(1.0_dpkind) * 4.0_dpkind,
     &                           rerth = 6371200.0_dpkind

      namelist /rsmdomain/ rproj, rtruth, rorient, rdelx, rdely,
     &                     rcenlat, rcenlon, rlftgrd, rbtmgrd

      u = 40

      open(unit=u, action='read', file='rsm-domain.nml', status='old',
     &     iostat=ios)

      if (ios /= 0) then
        print *, 'Failed to open namelist file rsmdomain.nml'
        stop
      end if

      read(unit=u, nml=rsmdomain, iostat=ios)

      if (ios /= 0) then
        print *, 'Failed to read namelist file rsmdomain.nml'
        stop
      end if
...
! if proj ==-1, do south polar projection
      ...
      radlat = nproj * radlat
      radlon = nproj * radlon
      yyy = a2 * cos(radlat) / (1.0 + sin(radlat))
      cenlat = rcenlat
      if (abs(cenlat) .eq. 90.) yyy = 0.0
      y00 = yyy * sin(radlon) - (rbtmgrd - 1.0) * dely
      x00 = yyy * cos(radlon) - (rlftgrd - 1.0) * delx
...
! fix problem which domain covers 0E.
          if (flons .lt. flon(ijlats + 1)) flon(ijlats + i) =
     &        flons + twopi
 200  continue
      endif
