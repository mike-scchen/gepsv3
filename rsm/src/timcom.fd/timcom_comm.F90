...
subroutine comm_init(mpi_comm_drv)
  implicit none

  integer, intent(in), optional :: mpi_comm_drv
  integer ierr

  if(present(mpi_comm_drv)) then
    m_comm_timcom = mpi_comm_drv
  else
    call MPI_INIT(ierr)
  end if
  ...
  call MPI_FILE_OPEN(m_comm_timcom, 'tai_runtime.log',  &
