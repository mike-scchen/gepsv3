...
!...................................
!  ---  inputs:
     &     ( im, ix, km, ps, u1, v1, t1, q1, soiltyp, vegtype, sigmaf,      &
     &       sfcemis, dlwflx, dswsfc, snet, delt, tg3, cm, ch,          &
     &       prsl1, prslki, zf, islimsk, ddvel, slopetyp,               &
     &       shdmin, shdmax, snoalb, sfalb, flag_iter, flag_guess,      &
     &       lheatstrg,isot, ivegsrc,                                   &
!yj:     &       bexppert, xlaipert, vegfpert,pertvegf,                     &  ! sfc perts, mgehne
!  ---  in/outs:
     &       weasd, snwdph, tskin, tprcp, srflag, smc, stc, slc,        &
     &       canopy, trans, tsurf, zorl,                                &
!  ---  outputs:
...
!
      ...
      use rphyscons, only : grav   => con_g,    cp   => con_cp,          &
     &                     hvap   => con_hvap, rd   => con_rd,          &
     &                     eps    => con_eps, epsm1 => con_epsm1,       &
     &                     rvrdm1 => con_fvirt

!yj:      use module_radiation_surface, only : ppfbet
...
!  ---  input:
      integer, intent(in) :: im, ix, km, isot, ivegsrc
!yj:      real (kind=kind_phys), dimension(6), intent(in) :: pertvegf

      integer, dimension(ix), intent(in) :: soiltyp, vegtype, slopetyp
     ...
     &       snoalb, sfalb, zf
!yj:     &       bexppert, xlaipert, vegfpert

      integer, dimension(ix), intent(in) :: islimsk
      real (kind=kind_phys),  intent(in) :: delt

      logical, dimension(ix), intent(in) :: flag_iter, flag_guess
      logical, intent(in) :: lheatstrg

!  ---  in/out:
      real (kind=kind_phys), dimension(ix), intent(inout) :: weasd,     &
     &       snwdph, tskin, tprcp, srflag, canopy, trans, tsurf

      real (kind=kind_phys), dimension(ix,km), intent(inout) ::         &
     &       smc, stc, slc

!  ---  output:
      real (kind=kind_phys), dimension(ix), intent(out) :: sncovr1,     &
     &       qsurf, gflux, drain, evap, hflx, ep, runoff, cmm, chh,     &
     &       evbs, evcw, sbsno, snowc, stm, snohf, smcwlt2, smcref2,    &
     &       zorl, wet1
    
!  ---  locals:
      real (kind=kind_phys), dimension(ix) :: rch, rho,                 &
     ...
     &       smsoil, slsoil
      ...
      real (kind=kind_phys), dimension(ix,km) :: zsoil, smc_old,        &
     &       stc_old, slc_old
...
!    srflag  - flag for snow-rain detection (1.=all snow, 0.=all rain, 0-1 mixed)                
...
!yj:!  perturb vegetation fraction that goes into sflx, use the same
!yj:!  perturbation strategy as for albedo (percentile matching)
!yj:        vegfp  = vegfpert(i)                    ! sfc-perts, mgehne
!yj:        ! sfc perts, mgehne
!yj:        if (pertvegf(1)>0.0) then
!yj:                ! compute beta distribution parameters for vegetation fraction
!yj:                mv = shdfac
!yj:                sv = pertvegf(1)*mv*(1.-mv)
!yj:                alphav = mv*mv*(1.-mv)/(sv*sv)-mv
!yj:                betav  = alphav*(1.-mv)/mv
!yj:                ! compute beta distribution value corresponding
!yj:                ! to the given percentile albPpert to use as new albedo
!yj:                call ppfbet(vegfp,alphav,betav,iflag,vegftmp)
!yj:                shdfac = vegftmp
!yj:        endif
!yj:! *** sfc-perts, mgehne
!yj:
...
!  ---- ... outside sflx, roughness uses cm as unit
          z0 = zorl(i)/100.
!yj:!  ---- mgehne, sfc-perts
!yj:          bexpp  = bexppert(i)                   ! sfc perts, mgehne
!yj:          xlaip  = xlaipert(i)                   ! sfc perts, mgehne
...
!yj:     &       bexpp, xlaip,                                              & ! sfc-perts, mgehne
     &       lheatstrg,                                                 &
!  ---  input/outputs:
