...
!!  @{
!-----------------------------------
      subroutine rsfc_sice                                               &
!...................................
!  ---  inputs:
     &     ( im, ix, km, ps, u1, v1, t1, q1, delt,                          &
...
!
      ...
      use rphyscons, only : sbc => con_sbc, hvap => con_hvap,            &
...
!  ---  inputs:
      integer, intent(in) :: im, ix, km, lsm, ipr
     ...
     &       prsl1, prslki, ddvel
      ...
      logical, intent(in) :: flag_iter(ix), mom4ice

!  ---  input/outputs:
      real (kind=kind_phys), dimension(ix), intent(inout) :: hice,      &
     &       fice, tice, weasd, tskin, tprcp, ep

      real (kind=kind_phys), dimension(ix,km), intent(inout) :: stc

!  ---  outputs:
      real (kind=kind_phys), dimension(ix), intent(out) :: snwdph,      &
     &       qsurf, snowmt, gflux, cmm, chh, evap, hflx

!  ---  locals:
      real (kind=kind_phys), dimension(ix) :: ffw, evapi, evapw,        &
     &       sneti, snetw, hfd, hfi,                                    &
!    &       hflxi, hflxw, sneti, snetw, qssi, qssw, hfd, hfi, hfw,     &
     &       focn, snof, hi_save, hs_save,                 rch, rho,    &
     &       snowd, theta1

      real (kind=kind_phys) :: t12, t14, tem, stsice(ix,kmi)
