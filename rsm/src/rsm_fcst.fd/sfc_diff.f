      subroutine rsfc_diff(im,ix,ps,u1,v1,t1,q1,z1,
     &                    snwdph,tskin,z0rl,cm,ch,rb,
     &                    prsl1,prslki,islimsk,
     &                    stress,fm,fh,zvfun,parea,ssu,ssv,
     &                    ustar,wind,ddvel,fm10,fh2,
     &                    sigmaf,vegtype,shdmax,ivegsrc,
!yj:     &                    z0pert,ztpert,                        ! mg, sfc-perts
...
!
     ...
     &,             eps => con_eps, epsm1 => con_epsm1
      ...
      implicit none
!
      integer              im, ix, ivegsrc
      real(kind=kind_phys), dimension(ix) :: ps,  u1, v1, t1, q1, z1
     &,                                      tskin, z0rl, cm,  ch, rb
     &,                                      prsl1, prslki, stress
     &,                                      fm, fh, ssu, ssv
     &,                                      ustar, wind, ddvel
     ...
     &,                                      tsurf, snwdph
!yj:     &,                                      z0pert,ztpert               ! mg, sfc-perts
      integer, dimension(ix)              ::  vegtype, islimsk

      logical   flag_iter(ix) ! added by s.lu
...
!>>CCPPv6.0.0
     &              charnock=.018, ca=.4    ! ca - von karman constant
!<<CCPPv6.0.0
     ...
     &,             ztmin1=-999.0
!>>CCPPv6.0.0
      real(kind=kind_phys), dimension(ix) :: zvfun,parea
      real(kind=kind_phys), parameter :: z0lo=0.1,z0up=1.0
     &                    , zolmin=-10.0, zmin=1.0e-6
     &                    , xkgdx=3000.0, xkmin=0.05, xkrefsqr=0.3
      real(kind=kind_phys) gdx, xkzo, tv1, zolmax
!<<CCPPv6.0.0
...
!               z0max = exp( tem2*log01 + tem1*log01 )
                z0max = 0.01
!     barren or sparsely vegetation mosaic
...
!     broadleaf shrubs with perennial groundcover
                elseif (vegtype(i) == 8) then
                  z0max = exp( tem2*log01 + tem1*log05 )
!     boradleaf shrubs with bare soil
...
!yj:! mg, sfc-perts: add surface perturbations to z0max over land
!yj:            if ( islimsk(i) == 1 .and. z0pert(i) /= 0.0 ) then
!yj:              z0max = z0max * (10.**z0pert(i))
!yj:            endif
!yj: 
...
!>>CCPPv6.0.0
            czilc = 10.0 ** (-4.0 * z0max) ! Trier et al. (2011,WAF)
            czilc = max(min(czilc,0.8),0.08)
            tem1 = 1.0 - sigmaf(i)
            czilc = czilc * tem1 * tem1
            ztmax = z0max*exp( - czilc*ca*258.2*sqrt(ustar(i)*z0max) )

!
! compute a function of surface roughness & green vegetation fraction (zvfun)
!
          if(islimsk(i) == 1) then            ! over land
...
!yj:! mg, sfc-perts: add surface perturbations to ztmax/z0max ratio over land
!yj:            if ( islimsk(i) == 1  .and. ztpert(i) /= 0.0) then
!yj:              ztmax = ztmax * (10.**ztpert(i))
!yj:            endif
...
!>>CCPPv6.0.0 (inclusion of moleculr viscosity effect in monentum
!roughness length (z0) (NCEP office note 505))
            tem1 = 0.11 * vis /ustar(i)
            z0 = tem1 + (charnock/grav) * ustar(i) * ustar(i)
!<<CCPPv6.0.0
