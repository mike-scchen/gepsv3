!>  \file mfpbl.f
!!  This file contains the subroutine that calculates the updraft properties
!!  and mass flux for use in the Hybrid EDMF PBL scheme.

!>  \ingroup PBL
!!  \brief This subroutine is used for calculating the mass flux and updraft
!!   properties.
!!
!!  The mfpbl routines works as follows: if the PBL is convective, first,
!!  the ascending parcel entrainment rate is calculated as a function of
!!  height. Next, a surface parcel is initiated according to surface layer
!!  properties and the updraft buoyancy is calculated as a function of height.
!!  Next, using the buoyancy and entrainment values, the parcel vertical velocity
!!  is calculated using a well known steady-state budget equation. With the
!!  profile of updraft vertical velocity, the PBL height is recalculated as
!!  the height where the updraft vertical velocity returns to 0, and the 
!!  entrainment profile is updated with the new PBL height. Finally, the 
!!  mass flux profile is calculated using the updraft vertical velocity and
!!  assumed updraft fraction and the updraft properties are calculated using
!!  the updated entrainment profile, surface values, and environmental profiles.
...
!
      real(kind=kind_phys) dt2,     dz,      factor,   bb1,     bb2,
     &                     xmmx,    rbint,
!     &                     rbint,   tau,
     &                     tem,     tem1,    tem2,
     &                     ptem,    ptem1,   ptem2
...
!  physical parameters
     ...
     &          gocp=g/cp,
!     &          ce0=0.37,qmin=1.e-8,alp=1.0,pgcon=0.55
     &          ce0=0.38,qmin=1.e-8,alp=1.0,pgcon=0.55,
     &          a1=0.08,b1=0.5,f1=0.15,c1=0.3,d1=2.58,tau=500.,
     &          zfmin=1.e-8,h1=0.33333333
!
!-----------------------------------------------------------------------
!
!************************************************************************
!
      kmpbl = km/2 + 1
      dt2 = delt
!> Since the mfpbl subroutine is called regardless of whether the PBL is
!  convective, a check of the convective PBL flag is performed and the 
!  subroutine returns back to moninedmf (with the output variables set to 
!  the initialized values) if the PBL is not convective.
      totflg = .true.
      do i=1,im
...
!!
      ...
      enddo
!>  ## Determine an updraft parcel's entrainment rate, buoyancy, and vertical velocity.
!!  Calculate the entrainment rate according to equation 16 in Siebesma et al. (2007)
!!  \cite siebesma_et_al_2007 for all levels (xlamue) and a default entrainment rate
!  (xlamax) for use above the PBL top.
      do i=1,im
        if(cnvflg(i)) then
          k = kpbl(i) / 2
          k = max(k, 1)
...
!
!>  Using equations 17 and 7 from Siebesma et al (2007) \cite siebesma_et_al_2007
!   along with \f$u_*\f$, \f$w_*\f$, and the previously diagnosed PBL height, the
!   initial \f$\theta_v\f$ of the updraft (and its surface buoyancy) is calculated.
      do i=1,im
        if(cnvflg(i)) then
          tem = zl(i,1)/hpbl(i)
...
!
!>  From the second level to the middle of the vertical domain, the updraft
!   virtual potential temperature is calculated using the entraining updraft
!   equation as in equation 10 of Siebesma et al (2007) \cite siebesma_et_al_2007,
!   discretized as
!!  \f[
!!  \frac{\theta_{v,u}^k - \theta_{v,u}^{k-1}}{\Delta z}=-\epsilon^{k-1}
!!  \left[\frac{1}{2}\left(\theta_{v,u}^k + \theta_{v,u}^{k-1}\right)-
!!  \frac{1}{2}\left(\overline{\theta_{v}}^k + \overline{\theta_v}^{k-1}\right)\right]
!!  \f]
!!  where the superscript \f$k\f$ denotes model level, and subscript \f$u\f$ denotes
!!  an updraft property, and the overbar denotes the grid-scale mean value.
      do k = 2, kmpbl
        do i=1,im
          if(cnvflg(i)) then
            dz = zl(i,k) - zl(i,k-1)
...
!
!>  Rather than use the vertical velocity equation given as equation 15 in
!   Siebesma et al (2007) \cite siebesma_et_al_2007 (which parameterizes the
!   pressure term in terms of the updraft vertical velocity itself), this
!   scheme uses the more widely used form of the steady state vertical velocity
!   equation given as equation 6 in Soares et al. (2004) \cite soares_et_al_2004
!   discretized as
!!  \f[
!!  \frac{w_{u,k}^2 - w_{u,k-1}^2}{\Delta z} = -2b_1\frac{1}{2}\left(\epsilon_k +
!!  \epsilon_{k-1}\right)\frac{1}{2}\left(w_{u,k}^2 + w_{u,k-1}^2\right) + 2b_2B
!!  \f]
!! The constants used in the scheme are labeled \f$bb1 = 2b_1\f$ and \f$bb2 = 2b_2\f$
!! and are tuned to be equal to 1.8 and 3.5, respectively, close to the values
!! proposed by Soares et al. (2004) \cite soares_et_al_2004 .
!     tem = 1.-2.*f1
!     bb1 = 2. * b1 / tem
!     bb2 = 2. / tem
