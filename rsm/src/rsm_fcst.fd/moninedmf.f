...
!!  @{
      subroutine rmoninedmf(ix,im,km,ntrac,ntcw,dv,du,tau,rtg,
     &   u1,v1,t1,q1,swh,hlw,xmu,
     &   psk,rbsoil,zorl,u10m,v10m,fm,fh,
     &   tsea,    heat,evap,stress,spd1,kpbl,
!    &   tsea,qss,heat,evap,stress,spd1,kpbl,
     &   prsi,del,prsl,prslk,phii,phil,delt, 
     &   dusfc,dvsfc,dtsfc,dqsfc,hpbl,hgamt,hgamq,gust,uvdiff)
!
      use rmachine  , only : kind_phys
      use rphyscons, grav => con_g, rd => con_rd, cp => con_cp,
     &              hvap => con_hvap, fv => con_fvirt
      implicit none
!
!     arguments
!
!xb118      integer ix, im, km, ntrac, ntcw, kpbl(im), kinver(im)
      integer ix, im, km, ntrac, ntcw, kpbl(ix)
!
      real(kind=kind_phys) delt 
      real(kind=kind_phys) dv(ix,km),     du(ix,km),
     &                     tau(ix,km),    rtg(ix,km,ntrac),
     &                     u1(ix,km),     v1(ix,km),
...
!
      real(kind=kind_phys) bvf2,    dk,
     &                     dq1,     dsdz2,  dsdzq,  dsdzt,
     &                     dsdzu,   dsdzv,
     &                     dsig,    dt2,    dthe1,  dtodsd,
     &                     dtodsu,  dw2,    prnum,  qtend,
     &                     rbint,   rdt,    rdz,    ri,     rl2,
     &                     rone,    rzero,  spdk2,  sri,    zol1,
     &                     robn,    ttend,  utend,  ust3,   wst3,
...
!
     ...
     &         ,g=grav
     &         ,gocp=g/cp
     &         ,cont=cp/g,conq=hvap/g,conw=1.0/g               ! for del in pa
!     &         ,cont=1000.*cp/g,conq=1000.*hvap/g,conw=1000./g ! for del in kpa
     &         ,rlam=30.0,vk=0.4,vk2=vk*vk
     &         ,prmin=0.25,prmax=4.,zolcr=0.2,zolcru=-0.5
     &         ,dw2min=0.0001,dkmin=0.0,dkmax=1000.,rimin=-100.
     &         ,crbcon=0.25,crbmin=0.15,crbmax=0.35
     &         ,wfac=7.0,cfac=6.5,pfac=2.0,sfcfrac=0.1
!     &         ,qmin=1.e-8,xkzm=1.0,zfmin=1.e-8,aphi5=5.,aphi16=16.
     &         ,qmin=1.e-8,         zfmin=1.e-8,aphi5=5.,aphi16=16.
     &         ,tdzmin=1.e-3,qlmin=1.e-12,f0=1.e-4
     &         ,h1=0.33333333,h2=0.66666667
     &         ,cldtime=500.,xkzminv=0.3
!     &         ,cldtime=500.,xkzmu=3.0,xkzminv=0.3
!     &         ,gamcrt=3.,gamcrq=2.e-3,rlamun=150.0
     &         ,xkzm_m=1.0,xkzm_h=1.0,xkzm_s=1.0
     &         ,gamcrt=3.,gamcrq=0.,rlamun=150.0
     &         ,rentf1=0.2,rentf2=1.0,radfac=0.85
!     &         ,actei = 0.23
     &         ,actei = 0.7
      integer,parameter :: iun=84
!     &          zstblmax = 2500., qlcr=1.0e-5
!     &          zstblmax = 2500., qlcr=3.0e-5
!     &          zstblmax = 2500., qlcr=3.5e-5
!     &          zstblmax = 2500., qlcr=1.0e-4
     &         ,zstblmax = 2500., qlcr=3.5e-5
      dspheat=.true.
!
!-----------------------------------------------------------------------
...
!!  \f]
      ...
      do i = 1, im
        if(.not.flg(i)) then
          ...
          rbup(i) = (thvx(i,k)-thermal(i))*(g*zl(i,k)/thvx(i,1))/spdk2
          kpbl(i) = k
          flg(i)  = rbup(i) > crb(i)
        endif
      enddo
...
!          phih(i) = (1.-aphi16*zol1)**(-1./2.)
         ...
         tem2 = (1. - (0.5/12.)* 
     &    (rbsoil(i)*phim(i)*phim(i)/phih(i))*10./hpbl(i) )**(1./3.)
         gust(i) = max(gust(i),tem1 + 7.71*ustar(i)*tem2 + 
...
!>  For details of the mfpbl subroutine, step into its documentation ::mfpbl
      call rmfpbl(im,ix,km,ntrac,dt2,pcnvflg,
...
!>  The tridiagonal system is solved by calling the internal ::tridin subroutine.
      call tridin_m(im,km,ntrac,al,ad,au,a1,a2,au,a1,a2)
...
!
      call tridi2_m(im,km,al,ad,au,a1,a2,au,a1,a2)
!
...
!!  Origin of subroutine unknown.
      subroutine tridi2_m(l,n,cl,cm,cu,r1,r2,au,a1,a2)             
!c
      use rmachine     , only : kind_phys
      implicit none
      integer             k,n,l,i
...
!!
!!  Origin of subroutine unknown.
      subroutine tridin_m(l,n,nt,cl,cm,cu,r1,r2,au,a1,a2)
!c
      use rmachine     , only : kind_phys
      implicit none
