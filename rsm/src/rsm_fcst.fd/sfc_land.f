!-----------------------------------
      subroutine rsfc_land                                               &
!...................................
!  ---  inputs:
     &     ( im, ix, km, ps, u1, v1, t1, q1, smc, soiltyp,                  &
...
!
      ...
      use rphyscons, only : grav => con_g, sbc => con_sbc, cp => con_cp, &
...
!  ---  input:
      ...
      integer, dimension(ix), intent(in) :: islimsk, soiltyp, vegtype
...
!    &      t1, q1, sigmaf, sfcemis, dlwflx, swnet, zorl, tg3, cm, ch,  &
     &      prsl1, prslki, ddvel
      ...
      real (kind=kind_phys), dimension(ix,km), intent(in) :: smc

      real (kind=kind_phys), intent(in) :: delt

      logical, intent(in) :: flag_iter(ix), flag_guess(ix)

!  ---  in/out:
      real (kind=kind_phys), dimension(ix),    intent(inout) :: weasd,  &
     &       tskin, tprcp, srflag, canopy, tsurf

      real (kind=kind_phys), dimension(ix,km), intent(inout) :: stc
...
!  ---  output:
     ...
     &       tref
      ...
      real (kind=kind_phys), dimension(ix,km), intent(out) :: zsoil,    &
     &       rhsmc, aim, bim, cim
...
!  ---  locals:
      real (kind=kind_phys), dimension(ix) :: weasd_old, tprcp_old,     &
     ...
     &       tsea, xx, yy, zz, kt1, kt2
      ...
      real (kind=kind_phys), dimension(ix,km) :: stc_old, et, stsoil,   &
     &       ai, bi, ci, rhstc
