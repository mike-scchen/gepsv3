...
#
# Try to find NetCDF includes and library. Supports static and shared libaries
# and allows each component to be found in sepearte prefixes.
#
# This module defines
#
...
# * NetCDF_PARALLEL             - Boolean True if NetCDF4 has parallel IO
#   support via hdf5 and/or pnetcdf
# * NetCDF_HAS_PNETCDF          - Boolean True if NetCDF4 has pnetcdf support
#
# Deprecated Defines - NetCDF_LIBRARIES            - [Deprecated] Use
# NetCDF::NetCDF_<LANG> targets instead.
#
...
# * NetCDF_<comp>_LIBRARY_SHARED - Boolean is true if libraries for component
#   are shared
# * NetCDF_<comp>_INCLUDE_DIRS  - the include directories for specified
#   component
# * NetCDF::NetCDF_<comp>       - target of component to be used with
#   target_link_libraries()
#
# The following paths will be searched in order if set in CMake (first priority)
# or environment (second priority)
#
# * NetCDF_ROOT                 - root of NetCDF installation
# * NetCDF_PATH                 - root of NetCDF installation
#
# The search process begins with locating NetCDF Include headers.  If these are
# in a non-standard location, set one of the following CMake or environment
# variables to point to the location:
#
...
# Notes:
#
# * Use "NetCDF::NetCDF_<LANG>" targets only.  NetCDF_LIBRARIES exists for
#   backwards compatibility and should not be used. - These targets have all the
#   knowledge of include directories and library search directories, and a
#   single call to target_link_libraries will provide all these transitive
#   properties to your target.  Normally all that is needed to build and link
#   against NetCDF is, e.g.: target_link_libraries(my_c_tgt PUBLIC
#   NetCDF::NetCDF_C)
# * "NetCDF" is always the preferred naming for this package, its targets,
#   variables, and environment variables - For compatibility, some variables are
#   also set/checked using alternate names NetCDF4, NETCDF, or NETCDF4 -
#   Environments relying on these older environment variable names should move
#   to using a "NetCDF_ROOT" environment variable
# * Preferred component capitalization follows the CMake LANGUAGES variables:
#   i.e., C, Fortran, CXX - For compatibility, alternate capitalizations are
...
foreach(_comp ${${CMAKE_FIND_PACKAGE_NAME}_FIND_COMPONENTS})
  ...
  if(NOT _name_${_COMP})
    message(
      SEND_ERROR
        "Find${CMAKE_FIND_PACKAGE_NAME}: COMPONENT ${_comp} is not a valid component. Valid components: ${_possible_components}"
    )
  endif()
endforeach()
...
foreach(_comp IN ITEMS "_" "_C_" "_Fortran_" "_CXX_")
  foreach(_name IN ITEMS NetCDF4 NetCDF NETCDF4 NETCDF)
    foreach(_var IN ITEMS ROOT PATH)
      list(APPEND _search_hints ${${_name}${_comp}${_var}}
           $ENV{${_name}${_comp}${_var}})
      list(APPEND _include_search_hints ${${_name}${_comp}INCLUDE_DIR}
           $ENV{${_name}${_comp}INCLUDE_DIR} ${${_name}${_comp}INCLUDE_DIRS}
           $ENV{${_name}${_comp}INCLUDE_DIRS})
    endforeach()
  endforeach()
endforeach()
...
foreach(_comp IN LISTS _search_components)
  ...
  if(NetCDF_${_comp}_INCLUDE_FILE)
    get_filename_component(NetCDF_${_comp}_INCLUDE_FILE
                           ${NetCDF_${_comp}_INCLUDE_FILE} ABSOLUTE)
    get_filename_component(NetCDF_${_comp}_INCLUDE_DIR
                           ${NetCDF_${_comp}_INCLUDE_FILE} DIRECTORY)
...
set(NetCDF_INCLUDE_DIRS
    "${NetCDF_INCLUDE_DIRS}"
    CACHE STRING "NetCDF Include directory paths" FORCE)
...
foreach(_comp IN LISTS _search_components)
  ...
  find_program(
    NetCDF_${_comp}_CONFIG_EXECUTABLE
  ...
  message(
    DEBUG
    "NetCDF_${_comp}_CONFIG_EXECUTABLE: ${NetCDF_${_comp}_CONFIG_EXECUTABLE}")
endforeach()
...
if(_val MATCHES "^(yes)$")
  set(NetCDF_PARALLEL
      TRUE
      CACHE STRING "NetCDF has parallel IO capability via pnetcdf or hdf5."
            FORCE)
else()
  set(NetCDF_PARALLEL
      FALSE
      CACHE STRING "NetCDF has no parallel IO capability." FORCE)
endif()
...
foreach(_comp IN LISTS _search_components)
  ...
  get_filename_component(NetCDF_${_comp}_LIBRARY ${NetCDF_${_comp}_LIBRARY}
                         ABSOLUTE)
  set(NetCDF_${_comp}_LIBRARY
      ${NetCDF_${_comp}_LIBRARY}
      CACHE STRING "NetCDF ${_comp} library" FORCE)
  ...
  if(NetCDF_${_comp}_LIBRARY)
    if(NetCDF_${_comp}_LIBRARY MATCHES ".a$")
      ...
      if(NOT ${NetCDF_${_comp}_LIBRARY} IN_LIST NetCDF_LIBRARIES)
        ...
        message(
          DEBUG
          "Adding new netcdf library [${_comp}]: ${NetCDF_${_comp}_LIBRARY}")
      endif()
  ...
  netcdf_config(${NetCDF_${_comp}_CONFIG_EXECUTABLE} ${_${_comp}_libs_flag}
                _val)
  if(_val)
    set(NetCDF_${_comp}_LIBRARIES ${_val})
    if(NOT NetCDF_${_comp}_LIBRARY_SHARED AND NOT NetCDF_${_comp}_FOUND
    )# Static targets should use nc_config to get a proper link line with all
     # necessary static targets.
    ...
    if(NOT NetCDF_${_comp}_LIBRARY_SHARED)
      message(
        SEND_ERROR
          "Unable to properly find NetCDF.  Found static libraries at: ${NetCDF_${_comp}_LIBRARY} but could not run nc-config: ${NetCDF_CONFIG_EXECUTABLE}"
      )
    endif()
  endif()

  # Use nc-config to set per-component INCLUDE_DIRS variable if possible
  netcdf_config(${NetCDF_${_comp}_CONFIG_EXECUTABLE} ${_${_comp}_includes_flag}
                _val)
  if(_val)
  ...
  if(NetCDF_${_comp}_LIBRARIES AND NetCDF_${_comp}_INCLUDE_DIRS)
    ...
    if(NOT TARGET NetCDF::NetCDF_${_comp})
      ...
      set_target_properties(
        NetCDF::NetCDF_${_comp}
        PROPERTIES IMPORTED_LOCATION ${NetCDF_${_comp}_LIBRARY}
                   INTERFACE_INCLUDE_DIRECTORIES
                   "${NetCDF_${_comp}_INCLUDE_DIRS}"
                   INTERFACE_LINK_LIBRARIES ${NetCDF_${_comp}_LIBRARIES})
      if(NOT _comp MATCHES "^(C)$")
        target_link_libraries(NetCDF::NetCDF_${_comp}
                              INTERFACE NetCDF::NetCDF_C)
      endif()
      if(MPI_${_comp}_FOUND)
        target_link_libraries(NetCDF::NetCDF_${_comp}
                              INTERFACE MPI::MPI_${_comp})
      endif()
    endif()
  endif()
endforeach()
...
set(NetCDF_LIBRARIES
    "${NetCDF_LIBRARIES}"
    CACHE STRING "NetCDF library targets" FORCE)
...
if(NetCDF_INCLUDE_DIRS)
  if(NetCDF_C_CONFIG_EXECUTABLE)
    ...
    if(_vers)
      string(REGEX REPLACE ".* ((([0-9]+)\\.)+([0-9]+)).*" "\\1" NetCDF_VERSION
                           "${_vers}")
    endif()
  else()
...
foreach(_comp IN LISTS _search_components)
  if(NetCDF_${_comp}_FOUND)
    # Record found components to avoid duplication in NetCDF_LIBRARIES for
    # static libraries
    set(NetCDF_${_comp}_FOUND
        ${NetCDF_${_comp}_FOUND}
        CACHE BOOL "NetCDF ${_comp} Found" FORCE)
    # Set a per-package, per-component found variable to communicate between
...
if(${CMAKE_FIND_PACKAGE_NAME}_FOUND
   ...
   AND _new_search_components)
  message(
    STATUS
      "Find${CMAKE_FIND_PACKAGE_NAME} [${CMAKE_CURRENT_LIST_DIR}/FindNetCDF.cmake]:"
  )
  ...
  foreach(_comp IN LISTS _new_search_components)
    ...
    if(${CMAKE_FIND_PACKAGE_NAME}_${_arg_${_COMP}}_FOUND)
      ...
      if(NetCDF_${_comp}_LIBRARY_SHARED)
        message(
          STATUS
            "  - NetCDF::NetCDF_${_comp} [SHARED] [Root: ${_root}] Lib: ${NetCDF_${_comp}_LIBRARY} "
        )
      else()
        message(
          STATUS
            "  - NetCDF::NetCDF_${_comp} [STATIC] [Root: ${_root}] Lib: ${NetCDF_${_comp}_LIBRARY} "
        )
      endif()
    endif()
  endforeach()
...
foreach(_prefix NetCDF NetCDF4 NETCDF NETCDF4 ${CMAKE_FIND_PACKAGE_NAME})
  ...
  foreach(_comp ${_search_components})
    ...
    set(${_prefix}_${_comp}_FOUND
        ${${CMAKE_FIND_PACKAGE_NAME}_${_arg_comp}_FOUND})
    set(${_prefix}_${_COMP}_FOUND
        ${${CMAKE_FIND_PACKAGE_NAME}_${_arg_comp}_FOUND})
    set(${_prefix}_${_arg_comp}_FOUND
        ${${CMAKE_FIND_PACKAGE_NAME}_${_arg_comp}_FOUND})
