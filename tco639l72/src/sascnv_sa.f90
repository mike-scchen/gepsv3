      subroutine sascnv_sa(im,ix,km,jcap,delt,del,ps,prsl,phil,ql_all,   &
           q1,t1,u1,v1,dot,cldwrk,rn,kbot,ktop,kcnv,slimsk,garea,        &
...
!
      parameter(cm=1.0,d0=.01)
      parameter(c0s=.002,c1=.002)
...
!exponentially decreases with decreasing temperature from Han et al.'s
!(2017) \cite han_et_al_2017 equation 8.
      do k = 1, km
        do i = 1, im
...
!(CIN) according to Han et al.'s (2017) \cite han_et_al_2017 equation
!13.
      do k = 2, km1
        do i = 1, im
          if (cnvflg(i)) then
...
!(cinacr) which is inversely proportional to the large-scale vertical
!velocity.
      do i = 1, im
        if(cnvflg(i)) then
!
...
!
      do i = 1, im
         asqecflg(i) = cnvflg(i)
         if(asqecflg(i) .and. gdx(i) .lt. dxcrtas) then
            asqecflg(i) = .false.
         endif
      enddo
      do k = 1, km
...
! 
      do k = km1, 1, -1
        do i = 1, im
          if (asqecflg(i) .and. k.lt.jmin(i)) then
              dz = zi(i,k+1) - zi(i,k)
              if(k.ge.kbcon(i)) then
                 tem  = xlamde * dz
                 tem1 = 0.5 * xlamdd * dz
...
!velocity (wc) and the cloud depth. It is also proportional to the grid
!size (gdx).
      do i= 1, im
        if(cnvflg(i)) then
...
!cloud base mass flux as a function of the mean updraft velcoity for the
!grid sizes where the quasi-equilibrium assumption of Arakawa-Schubert
...
!destabilization as in equation 5 of Pan and Wu (1995) \cite
!pan_and_wu_1995 :
!! \f[
!!  \frac{\partial A}{\partial t}_{LS}=\frac{A^+-cA^0}{\Delta t_{LS}}
!! \f]
!! Here \f$A^0\f$ is set to zero following  Han et al.'s (2017) \cite
!han_et_al_2017 , implying that the instability is completely eliminated
!after the convective adjustment time, \f$\Delta t_{LS}\f$.
      do i= 1, im
        if(asqecflg(i)) then
...
!cloud base mass flux) as in equation 6 of Pan and Wu (1995) \cite
!pan_and_wu_1995 :
!! \f[
...
!
!> - The cloud base mass flux (xmb) is then calculated from equation 7
!of Pan and Wu (1995) \cite pan_and_wu_1995
!! \f[
!! M_c=\frac{-\frac{\partial A}{\partial t}_{LS}}{\frac{\partial
!A}{\partial t}_{cu}}
!! \f]
!!
!!  Again when dtconv is larger than tauadv, the cloud base mass flux is
!further reduced in proportion to the ratio of tauadv to dtconv.
        if(asqecflg(i)) then
          tfac = tauadv(i) / dtconv(i)
          tfac = min(tfac, 1.)
