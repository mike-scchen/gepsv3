      subroutine gpvs_gpu(c1xpvs,c2xpvs,tbpvs,async_id)
      !$acc routine(fpvsx_gpu) seq
!
...
!
      ...
      real c1xpvs,c2xpvs,tbpvs(nxpvs),fpvsx_gpu
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      xmin=180.0
      xmax=330.0
...
!   c1xpvs=1.-xmin/xinc
      ...
      !$acc parallel loop gang vector private(jx, x) async(async_id)
      do jx=1,nxpvs
        x=xmin+(jx-1)*xinc
        tbpvs(jx)=fpvsx_gpu(x)
      enddo
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      end subroutine

      function fpvsx_gpu(t)
      !$acc routine seq
!
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      tr=ttp/t
      if(t.ge.tliq) then
        fpvsx_gpu=psat*(tr**xponal)*exp(xponbl*(1.-tr))
      elseif(t.lt.tice) then
        fpvsx_gpu=psat*(tr**xponai)*exp(xponbi*(1.-tr))
      else
        w=(t-tice)/(tliq-tice)
        pvl=psat*(tr**xponal)*exp(xponbl*(1.-tr))
        pvi=psat*(tr**xponai)*exp(xponbi*(1.-tr))
        fpvsx_gpu=w*pvl+(1.-w)*pvi
      endif
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      end function
