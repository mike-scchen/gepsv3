...
!
      real                 rdzt(im,km-1),dktx(im,km-1),dkux(im,km-1),   &
...
!     real(kind=kind_phys) prinv(im), hpbl01(im), rent(im)
      real                 prinv(im), rent(im),wscalex(im)
!
      logical  pblflg(im), sfcflg(im), scuflg(im), flg(im) ,ice(im)
!
      real                 aphi16,  aphi5,  bvf2,   wfac,               &
                           cfac,    conq,   cont,   conw,               &
                           dk,      dkmax,  dkmin,                      &
...
!                          rbint,   rdt,    rdz,    rdzt1,
                           ...
                           ptem,    ptem1,  ptem2
!
      real                 zstblmax,h1,     h2,     qlcr,  actei,       &
...
!     io=320
!     jo=320
!     if(j.eq.jo)print*,'wscale_old=',wscale(io)
!     if(j.eq.jo)print*,'wscale=',wscale(io)
!
...
!
      do k = 1, kmpbl
      do i=1,im
!org     if(pblflg(i).and.k.lt.kpbl(i)) then
         if(k.lt.kpbl(i)) then
! old
...
!     jo=320
!     if(j.eq.jo)then
!     print*,'(new_modi)dkt       ckt'
!     do k=kmpbl,1,-1
!     print*,'k=',k,dkt(io,k),ckt(io,k)
!     enddo
!     endif
!
      do k = 1, kmpbl
...
!     jo=320
!     if(j.eq.jo)then
!     print*,'new-pbl  ttend   dk  ck'
!     do k=km,1,-1
!     print*,'k=',k,a1(io,k),tau(io,k),dkt(io,k),ckt(io,k)
!     enddo
!     endif
!
!byl      if(ntrac.eq.2) then
