      subroutine landsfc_n ( nxj,mn,kk,dt,g,r,cp,hltm,tg,z0,ocean,ps,ts,hgt &
...
! for landsfc-layer
      ...
      real      fm(mn),fh(mn),hlinf(mn),olinf(mn),fm10(mn),fh2(mn)
      real      hl1(mn),pm(mn),ph(mn)
      real      restar(mn),rat(mn),pm10(mn),ph2(mn),               &
! for pm-evap.
...
!cc for output of t2, u10 and v10 ( 2003/01/20 )
      real  u10(mn),v10(mn),t2(mn),q2(mn),rh2(mn),p2(mn),wk2(mn)
      real  hl12x,hl110x,hl12,hl110
...
!       if(ncepsfc)z0(i) = 0.014 *ustar(i)*ustar(i)/g
...
!     t2(i)=t2(i)*exp(xkapa*log(p2/1000.))
...
!
        if(dtv(i).ge.0.) then
          ...
          ph(i) = bb0 - bb + log((bb + 1.) / (bb0 + 1.))
!
          hl110 = hl1(i) * 10. /hgt(i,kk)
          aa = sqrt(1. + 4. * alpha * hl110)
          pm10(i) = aa0 - aa + log((aa + 1.) / (aa0 + 1.))
          hl12 = hl1(i) * 2. /hgt(i,kk)
          aa = sqrt(1. + 4. * alpha * hl12)
          bb = sqrt(1. + 4. * alpha * hl12)
...
!
          ...
          HL12 = HL1(I) * 2. / HGT(I,KK)
          PH2(I) = (A0P + A1P * HL12) * HL12 &
                  / (1. + B1P * HL12 + B2P * HL12 * HL12)
        endif
...
!         PM10(I) = LOG(HL110) + 2. * HL110 ** (-.25) - .8776
          PM10(I) = LOG(HL110) + 2. / (sqrt(hl110x)) - .8776
!
          HL12 = HL1(I) * 2. / HGT(I,KK)
          hl12x=sqrt(hl12)
!         PH2(I) = LOG(HL12(I)) + .5 * HL12(I) ** (-.5) + 1.386
          ph2(i) = log(hl12) + .5/hl12x + 1.386
...
!     if(.not. ocean(i))then
      ...
      xx1    = phih(i)*dt*sfcw(i) / (2.0*hgt(i,kk))
      wt(i) = ( tt(i,kk)*(1.0-al*xx1)+xx1*tg(i)/pk2(i,kk) )/(1.0+bl*xx1)
      wq(i) = ( qt(i,kk)*(1.0-al*xx1) + xx1*qsfc(i) ) / (1.0+bl*xx1)
!     endif
      enddo
...
!     if(.not. ocean(i))then
      ...
      wt(i) = 0.5*(wt(i)+tt(i,kk))
      wq(i) = 0.5*(wq(i)+qt(i,kk))
      sfcw(i) =max( 1.0e-2,sqrt( wu(i)*wu(i)+wv(i)*wv(i) ))
      ustar(i)=sqrt(phim(i)*sfcw(i)*sfcw(i))
      tstar(i)=phih(i)*sfcw(i)*(wt(i)*pk2(i,kk)-tg(i))/ustar(i)
      qstar(i)=phih(i)*sfcw(i)*(wq(i)-qsfc(i))/ustar(i)
!     endif
      enddo
!
      do i=1,nxj
      fm10(i)= max(fm10(i),1.0e-3)
      fh2(i)= max(fh2(i),1.0e-3)
!
      v10(i)=wv(i)*fm10(i)/fh(i)
      u10(i)=wu(i)*fm10(i)/fm(i)
!
      q2(i)=qsfc(i)*(1.-fh2(i)/fh(i))+wq(i)*fh2(i)/fh(i)
      t2(i)=tg(i)/pk2(i,kk)*(1.-fh2(i)/fh(i))+wt(i)*fh2(i)/fh(i)
!
! convert t2 from potential temp. to temp
      p2(i)=(ps(i)*100.-rosfc(i)*g*2.)/100.
      t2(i)=t2(i)*(p2(i)/1000.)**xkapa
...
!
       ...
       if(.not. ocean(i))then
        if(flagsnw(i))then
         ...
         gfx(i)=-dft0(i)*(tsurf(i)-stsoil(i,1))/ &
                (-0.5*zsoil(i,1))
        endif
       gfx(i)=max(gfx(i),-200.)
       gfx(i)=min(gfx(i),200.)
...
!snow melt rate
        ...
        do i=1,nxj
         ...
         snomt(i)=rch(i)*rsmall(i)*(tsurf(i)-t0c)/(rhoh2o*hfus)*1000.
...
!
  ...
  500 continue
      do 520 i = 1, nxj
      zl(i) =g04*hgt(i,kk)*tstar(i)/     &
              (wt(i)*ustar(i)*ustar(i))
