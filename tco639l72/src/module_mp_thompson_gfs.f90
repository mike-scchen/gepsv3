...
!     REAL, PARAMETER, PRIVATE:: Nt_c = 100.E6
      REAL, PARAMETER, PRIVATE:: Nt_co = 100.E6
      REAL, PARAMETER, PRIVATE:: Nt_cl = 300.E6
...
!     REAL, PRIVATE::  ocg1, ocg2
      REAL, DIMENSION(3,2), PRIVATE:: cce, ccg
      REAL, DIMENSION(2), PRIVATE::  ocg1, ocg2
...
!     if (.NOT. ALLOCATED(tni_qcfz)) ALLOCATE(tni_qcfz(ntb_c,45))
      if (.NOT. ALLOCATED(tpi_qcfz)) ALLOCATE(tpi_qcfz(ntb_c,45,2))
      if (.NOT. ALLOCATED(tni_qcfz)) ALLOCATE(tni_qcfz(ntb_c,45,2))
...
!     mu_c = MIN(15., (1000.E6/Nt_c + 2.))
      mu_co = MIN(15., (1000.E6/Nt_co + 2.))
      mu_cl = MIN(15., (1000.E6/Nt_cl + 2.))
...
!     ocg2 = 1./ccg(2)
      cce(1,1) = mu_cl + 1.
      cce(1,2) = mu_co + 1.
      cce(2,1) = bm_r + mu_cl + 1.
      cce(2,2) = bm_r + mu_co + 1.
      cce(3,1) = bm_r + mu_cl + 4.
      cce(3,2) = bm_r + mu_co + 4.
      ccg(1,1) = WGAMMA(cce(1,1))
      ccg(1,2) = WGAMMA(cce(1,2))
      ccg(2,1) = WGAMMA(cce(2,1))
      ccg(2,2) = WGAMMA(cce(2,2))
      ccg(3,1) = WGAMMA(cce(3,1))
      ccg(3,2) = WGAMMA(cce(3,2))
      ocg1(1) = 1./ccg(1,1)
      ocg1(2) = 1./ccg(1,2)
      ocg2(1) = 1./ccg(2,1)
      ocg2(2) = 1./ccg(2,2)
...
!        enddo
         do j = 1, 2
         do i = 1, ntb_c
            tpi_qcfz(i,k,j) = 0.0d0
            tni_qcfz(i,k,j) = 0.0d0
         enddo
         enddo
      enddo
...
!+---+-----------------------------------------------------------------+
      SUBROUTINE mp_gt_driver(ims,ime,kms,kme,its,ite,kts,kte,        & 
		              qv, qc, qr, qi, qs, qg, ni, nr,         &
                              tt, p, dz, dt_in,itimestep,             &
                              RAINNCV,                                &
                              SR,islmsk,                              & 
                              refl_10cm,lradar,                       &
...
!     INTEGER, INTENT(IN):: ids,ide, kds,kde
      ...
      INTEGER, INTENT(IN):: itimestep,me

!..Local variables
      REAL, DIMENSION(kts:kte):: &
                          qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d, &
                          nr1d, t1d, p1d, dz1d, dBZ, nc1d 
...
!         if(me==0) then 
!           print*,'islmsk,Nt_c:',i,islmsk(i),Nt_c,Nt_cl,Nt_co
!         endif 

         do k = kts, kte
            t1d(k) = tt(i,k)
            p1d(k) = p(i,k)
            dz1d (k) = (phii(i,k+1)-phii(i,k)) / g 
!           qv1d(k) = qv(i,k)
            qv1d(k) = qv(i,k)/(1.-qv(i,k))
            qc1d(k) = qc(i,k)
            qi1d(k) = qi(i,k)
            qr1d(k) = qr(i,k)
            qs1d(k) = qs(i,k)
         ...
         pcp_gr(i) = pptgraul
         pcp_ic(i) = pptice
         RAINNCV(i) = (pptrain + pptsnow + pptgraul + pptice)/1000.
...
!         ENDIF
         SR(i) = (pptsnow + pptgraul + pptice)/(RAINNCV(i)*1000.+1.e-12)
        
         do k = kts, kte
!           qv(i,k) = qv1d(k)
            qv(i,k) = qv1d(k)/(1.+qv1d(k))
            qc(i,k) = qc1d(k)
            qi(i,k) = qi1d(k)
            qr(i,k) = qr1d(k)
            qs(i,k) = qs1d(k)
...
!                        ' at i,k=', i,k
         ...
         if (lradar) then
          ...
          call calc_effectRad (t1d, p1d, qv1d, qc1d, nc1d, qi1d, ni1d, qs1d, &
                      islmsk(i), re_qc1d, re_qi1d, re_qs1d, kts, kte)
          do k = kts, kte
             re_cloud(i,k)=MAX( 2.51E-6, MIN(re_qc1d(k), 50.E-6))*1.e6
             re_ice(i,k)  =MAX(10.01E-6, MIN(re_qi1d(k), 125.E-6))*1.e6
             re_snow(i,k) =MAX(10.01E-6, MIN(re_qs1d(k), 999.E-6))*1.e6
          enddo
      enddo i_loop

! DEBUG - GT
!      write(mp_debug,'(a,7(a,e13.6,1x,a,i3,a,i3,a,1x))') 'MP-GT:', &
!         'qc: ', qc_max, '(', imax_qc, ',', kmax_qc, ')', &
!         'qr: ', qr_max, '(', imax_qr, ',', kmax_qr, ')', &
!         'qi: ', qi_max, '(', imax_qi, ',', kmax_qi, ')', &
!         'qs: ', qs_max, '(', imax_qs, ',', kmax_qs, ')', &
!         'qg: ', qg_max, '(', imax_qg, ',', kmax_qg, ')', &
!         'ni: ', ni_max, '(', imax_ni, ',', kmax_ni, ')', &
!         'nr: ', nr_max, '(', imax_nr, ',', kmax_nr, ')'
!      CALL wrf_debug(150, mp_debug)
!      write(*,'(a,7(a,e13.6,1x,a,i3,a,i3,a,1x))') 'MP-GT:',         &
!         'qc: ', qc_max, '(', imax_qc, ',', kmax_qc, ')', &
!         'qr: ', qr_max, '(', imax_qr, ',', kmax_qr, ')', &
!         'qi: ', qi_max, '(', imax_qi, ',', kmax_qi, ')', &
!         'qs: ', qs_max, '(', imax_qs, ',', kmax_qs, ')', &
...
!..Local variables
      ...
      LOGICAL:: debug_flag
      INTEGER:: idx_lo ! land and ocean

!+---+

      debug_flag = .false.
!     if (ii.eq.315 ) debug_flag = .true.
...
!        lamc = (Nt_c*am_r* ccg(2) * ocg1 / rc(k))**obmr
         if(islmski == 1) then
           xDc = MAX(D0c*1.E6, ((rc(k)/(am_r*Nt_cl))**obmr) * 1.E6)
           lamc = (Nt_cl*am_r* ccg(2,1) * ocg1(1) / rc(k))**obmr ! land
         else
           xDc = MAX(D0c*1.E6, ((rc(k)/(am_r*Nt_co))**obmr) * 1.E6)
           lamc = (Nt_co*am_r* ccg(2,2) * ocg1(2) / rc(k))**obmr ! ocean
         endif
...
!        print*,'mvd_c:',mvd_c(k),islmski,mu_co,mu_cl,lamc
...
!          pri_wfz(k) = tpi_qcfz(idx_c,idx_tc)*odts
           pri_wfz(k) = tpi_qcfz(idx_c,idx_tc,idx_lo)*odts
           pri_wfz(k) = MIN(DBLE(rc(k)*odts), pri_wfz(k))
!          pni_wfz(k) = tni_qcfz(idx_c,idx_tc)*odts
           pni_wfz(k) = tni_qcfz(idx_c,idx_tc,idx_lo)*odts
...
!         print*, ' Freezing water for temp = ', -k
         ...
         do j = 1, ntb_r1
            ...
            enddo
         enddo
         do j = 1, 2  !land and ocean 
          do i = 1, ntb_c
!           lamc = 1.0D-6 * (Nt_c*am_r* ccg(2) * ocg1 / r_c(i))**obmr
!            N0_c = 1.0D-18 * Nt_c*ocg1 * lamc**cce(1)
                        if(j==1) then
              lamc = 1.0D-6 * (Nt_cl*am_r* ccg(2,1) * ocg1(1) / r_c(i))**obmr
              N0_c = 1.0D-18 * Nt_cl*ocg1(1) * lamc**cce(1,1)
            else
              lamc = 1.0D-6 * (Nt_co*am_r* ccg(2,2) * ocg1(2) / r_c(i))**obmr
              N0_c = 1.0D-18 * Nt_co*ocg1(2) * lamc**cce(1,2)
            endif
            sum1 = 0.0d0
            sumn2 = 0.0d0
            do n = nbc, 1, -1
...
!              N_c(n) = N0_c* y**mu_c * EXP(-lamc*y)*dtc(n)
               if(j==1) then
                 ...
                 N_c(n) = N0_c* y**mu_co * EXP(-lamc*y)*dtc(n)
               endif
               N_c(n) = 1.0D24 * N_c(n)
...
!+---+-----------------------------------------------------------------+
     ...
     &                islmski,re_qc1d, re_qi1d, re_qs1d, kts, kte)
