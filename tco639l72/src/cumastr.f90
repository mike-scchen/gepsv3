...
!                  ptte,pqte,pvom,pvol,prsfc,pssfc,paprc,paprs,pxtec,ldcum,ktype
!  ,kcbot, &
!                  kctop,ptu,pqu,plu,plude,pmfu,pmfd,prain,&
!                  ztodt,du,eu,ed)

!SUBROUTINE cumastr(klon,klev,klevp1,klevm1,pten,pqen,pxen, puen,pven,   &
SUBROUTINE cumastr(nxj,klon,klev,klevp1,klevm1,pten,pqen,pxen, puen,pven,   &
                   ldland,pverv,pqsen,pqhfl,papp1,paphp1,pgeo,   &
...
!  INTEGER, INTENT (IN) :: klev, klevm1, klevp1, klon
  INTEGER, INTENT (IN) :: klev, klevm1, klevp1, klon, nxj
  ...
  REAL(r8), INTENT (INOUT) ::            &
                          pqte(klon,klev), pxtec(klon,klev),       &
...
! LOGICAL, INTENT (INOUT) :: ldcum(klon)
  ...
  REAL(r8) :: zalvdcp, zb, zbi, zcons2, zdepth, zdhdz, zdqmin, zdz, zeps, zfac,  &
          ...
          ztmst, zzz
  INTEGER :: icum, ikb, itopm2, jk, jl, jin
  LOGICAL :: llo1

  !  Local arrays: 
  INTEGER :: ilab(klon,klev)
  REAL(r8) :: paprc(klon), paprs(klon), plu(klon,klev), &
              du(klon,klev),eu(klon,klev),        &
              ed(klon,klev), ptu(klon,klev),pqu(klon,klev),        &
              plude(klon,klev), pmfd(klon,klev), pmfu(klon,klev), prain(klon)
...
!  CALL cuini(klon,klev,klevp1,klevm1,pten,pqen,pqsen,pxen,puen,          &
  CALL cuini(nxj,klon,klev,klevp1,klevm1,pten,pqen,pqsen,pxen,puen,          &
             pven, &
     pverv,pgeo,paphp1,     &
             zgeoh,ztenh,zqenh,zqsenh,zxenh,ilwmin,ptu,pqu,ztd,zqd,zuu,zvu,zud,zvd, &
             pmfu,pmfd,zmfus,zmfds,zmfuq,zmfdq,zdmfup,zdmfdp,zdpmel,plu,plude,ilab,jin)
...
!  CALL cubase(klon,klev,klevp1,klevm1,ztenh,zqenh,zgeoh,paphp1, &
  CALL cubase(nxj,klon,klev,klevp1,klevm1,ztenh,zqenh,zgeoh,paphp1, &
              ptu,pqu,plu,puen,pven,zuu,zvu,ldcum,kcbot,ilab,jin)
...
!  CALL cuasc(klon,klev,klevp1,klevm1,ztenh,zqenh,zxenh,puen,pven, &
  CALL cuasc(nxj,klon,klev,klevp1,klevm1,ztenh,zqenh,zxenh,puen,pven, &
...
!    CALL cudlfs(klon,klev,klevp1,ztenh,zqenh,puen,pven,&
    CALL cudlfs(nxj,klon,klev,klevp1,ztenh,zqenh,puen,pven,&
...
!    CALL cuddraf(klon,klev,klevp1,ztenh,zqenh,puen,pven,&
    CALL cuddraf(nxj,klon,klev,klevp1,ztenh,zqenh,puen,pven,&
...
! ztau is the time for the deep convection to consume the cape
! A smaller value will give larger cloud-base flux.
! ztau = 3.*3600.
!  ztau = 3600.
  ztau= 1800.
!  DO jl = 1, klon
  DO jl = 1, nxj
    IF (ldcum(jl) .AND. ktype(jl)==1) THEN
      ikb = kcbot(jl)
! comment out to remove the CAPE factor in the Tiedtke-Nordeng scheme
...
!  CALL cuflx(klon,klev,klevp1,pqen,pqsen,ztenh,zqenh,&
  CALL cuflx(nxj,klon,klev,klevp1,pqen,pqsen,ztenh,zqenh,&
...
!  CALL cudtdq(klon,klev,klevp1,itopm2,paphp1,ldcum,pten,ptte,pqte,&
  CALL cudtdq(nxj,klon,klev,klevp1,itopm2,paphp1,ldcum,pten,ptte,pqte,&
...
!    CALL cududv(klon,klev,klevp1,itopm2,ktype,kcbot,paphp1,ldcum, &
    CALL cududv(nxj,klon,klev,klevp1,itopm2,ktype,kcbot,paphp1,ldcum, &
