...
MODULE module_mp_wsm6
   ...
   USE module_mp_radar
   use physcons,   only : cpd => con_cp, g => con_g, rd => con_rd,      &
                          rv => con_rv, xlv0 => con_hvap,               &
                          xlf0 => con_hfus,                             &
                          t0c => con_t0c,                               &
                          psat => con_psat,cliq => con_cliq,            &
                          cicec => con_csol, ep1 => con_fvirt,           &
                          ep2 => con_eps, cpv => con_cvap,              &
                          qmin => con_epsq,                             &
                          den0  => rhoair,                              &
                          denr  => rhowater, eps1 => con_fvirt
!
...
!rsun   REAL, PARAMETER, PRIVATE :: xncr = 3.e8        ! maritime cloud in contrast to 3.e8 in tc80
   REAL, PARAMETER, PRIVATE :: xncrl = 3.e8        ! land cloud in contrast to 3.e8 in tc80
   REAL, PARAMETER, PRIVATE :: xncro = 1.e8        ! maritime cloud in contrast to 3.e8 in tc80
...
!  REAL, PARAMETER, PRIVATE :: lamdagmax = 6.e4   ! limited maximum value for slope parameter of graupel
   ...
   REAL, SAVE ::                                      &
!rsun        qc0, qck1, pidnc,                        &  ! rsun 
             qc0o, qc0l,qck1o,qck1l, pidnc,           &  ! rsun 
...
!===================================================================
!
  SUBROUTINE wsm6(t, phii,qq, qc, qr, qi, qs, qg                   &
                 ,p, del                                           &
                 ,delt                                             &
                 ,rainncv                                          &
                 ,sr                                               &
  ...
  REAL, DIMENSION( ims:ime , kms:kme),                            &
        INTENT(INOUT) ::                                          &
                                                             ...
                                                             qq,  &
                                                              ...
                                                              qs, &
                                                              qg
  REAL, DIMENSION( ims:ime , kms:kme) ::                     den            
  REAL, DIMENSION( ims:ime , kms:kme),                            &
        INTENT(IN   ) ::                                          &
                                                               p, &
                                                            del
  ...
  REAL, DIMENSION(ims:ime, kms:kme),                              &
        INTENT(INOUT)::                                           &
                                                        re_cloud, &
                                                         ...
                                                         re_snow
!+---+-----------------------------------------------------------------+
  REAL, DIMENSION(ims:ime, kms:kme) ::                                 &  ! GT
                                                       refl_10cm
!+---+-----------------------------------------------------------------+
...
! LOCAL VAR
  ...
  REAL, DIMENSION( ims:ime , kms:kme) ::       q
  INTEGER ::               i,j,k
...
!
  ...
  REAL, DIMENSION( ims:ime , kms:kme ) ::                   delz
  REAL, INTENT(IN   ) ::                                    delt
!  REAL, INTENT(IN   ) ::                                    delt, &
!                                                               g, &
!                                                             cpd, &
...
!
      ...
      do k = kts, kte
        do i = its, ite
          delz(i,k) = (phii(i,k+1) - phii(i,k)) / g 
...
!      enddo
      do k = kts, kte
        do i = its, ite
          tvec1(i) = den0 / den(i,k)
          denfac(i,k) = sqrt(tvec1(i))
        enddo
      enddo
!
! Inline expansion for fpvs
!         qs(i,k,1) = fpvs(t(i,k),0,rd,rv,cpv,cliq,cicec,xlv0,xls,psat,t0c)
!         qs(i,k,2) = fpvs(t(i,k),1,rd,rv,cpv,cliq,cicec,xlv0,xls,psat,t0c)
      xls = xlv0 + xlf0 
      hsub = xls
      hvap = xlv0
      cvap = cpv
...
!
!--------------------------------------------------------------------------
      REAL FUNCTION fpvs(t,ice,rd,rv,cvap,cliq,cicec,hvap,hsub,psat,t0c)
!--------------------------------------------------------------------------
      IMPLICIT NONE
!--------------------------------------------------------------------------
      REAL t,rd,rv,cvap,cliq,cicec,hvap,hsub,psat,t0c,dldt,xa,xb,dldti,         &
           xai,xbi,ttp,tr
      INTEGER ice
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ttp=t0c+0.01
...
!
!rsun   qc0  = 4./3.*pi*denr*r0**3*xncr/den0  ! 0.419e-3 -- .61e-3
   qc0o  = 4./3.*pi*denr*r0**3*xncro/den0  ! 0.419e-3 -- .61e-3
   qc0l  = 4./3.*pi*denr*r0**3*xncrl/den0  ! 0.419e-3 -- .61e-3
!rsun   qck1 = .104*9.8*peaut/(xncr*denr)**(1./3.)/xmyu*den0**(4./3.) ! 7.03
   qck1o = .104*9.8*peaut/(xncro*denr)**(1./3.)/xmyu*den0**(4./3.) ! 7.03
   qck1l = .104*9.8*peaut/(xncrl*denr)**(1./3.)/xmyu*den0**(4./3.) ! 7.03
   pidnc = pi*denr/6.        ! syb
!
   bvtr1 = 1.+bvtr
   bvtr2 = 2.5+.5*bvtr
...
!-----------------------------------------------------------------------
     subroutine effectRad_wsm6 (t, qc, qi, qs, rho, qmin, t0c,        &
                                re_qc, re_qi, re_qs, islmski, kts, kte, ii)
