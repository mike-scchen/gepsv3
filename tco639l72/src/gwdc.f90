...
!-----------------------------------------------------------------------
      ...
      integer im, ix, iy, km, lat, ipr,j,iii,kkk,iinpt
...
!-----------------------------------------------------------------------
      ...
      integer i,ii,k,kk,kb,ilev,npt,kcb,kcldm,npr
...
!byl        k1 = km - k + 1
        do i=1,npt
          ...
          v(i,k)        = v1(ii,k)
          t(i,k)        = t1(ii,k)
          spfh(i,k)     = max(q1(ii,k),qmin)
          pmid(i,k)     = pmid1(ii,k)
          dpmid(i,k)    = dpmid1(ii,k) * onebg
...
!
        windcltop(i) = 1.0 / sqrt( ucltop(i)*ucltop(i) + vcltop(i)*vcltop(i) )
...
!byl        k1 = km - k + 1
        do i=1,npt
          ii = ipt(i)
          utgwc(ii,k) = utgwcl(i,k)
          vtgwc(ii,k) = vtgwcl(i,k)
!         brunm(ii,kk) = brunm(i,k)
!         brunm(i,k)  = tem
!         rhom(ii,kk) = rhom(i,k)
        enddo
