...
!***********************************************************************
! **** (Dyn core) input ****
! - pt, ut, vt, tt, qt, rdiv, phi, dtpl, dlpl
...
!
   real(kind=RTYPE) ndsldtah, dtah, dtahi, dta, &
      ...
      ptm(nxp, my_max), &
      deldm(nxp, my_max), sdpbl(nxp, my_max), cosl_j
...
#ifdef TIMING
! for timing
   real*8 tm_1, tm_2, tm_use, mpi_wtime, cputime1, cputime2
!CWB2020
   tm_1 = mpi_wtime()
   tm_2 = mpi_wtime()
#endif
   ...
   !$acc enter data copyin(ndslvvar) async(async_id)
   !$acc enter data copyin( &
   !$acc& lonlen, lonstr, latlen, &
   !$acc& gglati, fa1, fa2, fa3, fa4) async(async_id)
   !! << const >>
   !$acc enter data copyin(grav, rgas, cp, rad, radsq, sinl, cosl, &
   !$acc& cor, sigma, dsigma, ptop, hdk1, hdk2) async(async_id)
   ...
   ! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   !! << spec >>
   !$acc enter data create(temmid, divmid, vormid, plmid) async(async_id)
   !! << grid >>
   ...
   !$acc enter data create(ww1, pten, deldm, wkmf) async(async_id)
   !$acc enter data create(ut_sl, vt_sl, uum_sl, vvm_sl, ttm_sl) async(async_id)
   !$acc enter data create(qm_sl, pten_sl) async(async_id)
   !! << variables used in diabat_gpu >>
   !$acc enter data create(o3l, dtrad, asl, atl, asl_clr, atl_clr, e, eps, &
   !$acc&      cnvcr, cnvwr, ftp, fqp, ftp1, fqp1, clds, sld, rld, runoff, flash, &
   !$acc&      gfx, ugws, vgws, totallp, raincu, rainlp, raincu1, rainlp1, &
   !$acc&      raincu6, rainlp6, raintot) async(async_id)
   !$acc enter data copyin(dtcup, ducup, dvcup, dtshl, dushl, dvshl, dtlsp, dulsp, dvlsp) async(async_id)
   !$acc enter data copyin(stc, smc, slc, gwclim, tgclim, totalp, rs, sfalb, &
   !$acc&      sigmaf, istyp, ivegtyp, slopetyp) async(async_id)
   !$acc enter data copyin(land, ocean, ice, cosz, tg, tsflw, std, &
   !$acc&      ss, sfemis, shdmax, shdmin, snoalb, z0, ustar, snr, canopy, &
...
!
   !$acc host_data use_device(ttm_sl, pten_sl, qm_sl, vvm_sl, uum_sl)
   istat = cudaMemsetAsync(ttm_sl, real(0.0, RTYPE), size(ttm_sl), stream)
   istat = cudaMemsetAsync(pten_sl, real(0.0, RTYPE), size(pten_sl), stream)
   istat = cudaMemsetAsync(qm_sl, real(0.0, RTYPE), size(qm_sl), stream)
   istat = cudaMemsetAsync(vvm_sl, real(0.0, RTYPE), size(vvm_sl), stream)
   istat = cudaMemsetAsync(uum_sl, real(0.0, RTYPE), size(uum_sl), stream)
   !$acc end host_data
!
   lmax = 16
   nfxr = 33
...
!                    , km_soil,smc,slc,stc,canopy,zice,ggdef,gmdef )
   ...
   if (.not. restrt) then
      ...
      sld = 0.
      !$acc wait(async_id)
      !$acc update device(sld, rld) async(async_id)
      !$acc wait(async_id)
   end if
   recn = 1
   rdivm = 0.
   ...
   pdry = 0.
   !$acc wait(async_id)
   !$acc update device(flash, gfx, raincu1, rainlp1, raincu6, rainlp6, raintot) async(async_id)
   !$acc wait(async_id)
!
#ifdef TIMCOMCPL
      tocn_cpl = 0.
...
!
   ...
   !$acc wait(async_id)
   !$acc enter data copyin(hprime_b) async(async_id)
   !$acc wait(async_id)
!
   tchange = .false.
   nxmy = nx*my
...
!
   ...
   do jj = 1, jlistnum
      do k = 1, lev
         ...
         loc_sum3 = 0.
         loc_sum4 = 0.
         !$acc loop vector reduction(+:loc_sum1, loc_sum2, loc_sum3, loc_sum4)
         do i = 1, nxj
            dsigp = dsigma(k, 1)*pt(i, jj) + dsigma(k, 2)

            loc_sum1 = loc_sum1 + qt(i, k, jj)*dsigp*cosl_j
            loc_sum2 = loc_sum2 + tt(i, k, jj)*dsigp*cosl_j &
                       /(1.+0.608*qt(i, k, jj))
            loc_sum3 = loc_sum3 + (ut(i, k, jj)**2 + vt(i, k, jj)**2)*dsigp &
                       *radsq/(2.*cosl_j)
   ...
   !$acc end host_data
   !$acc exit data copyout(wk4) async(async_id)
!
   cosw = 0.
   do j = 1, my
      nxj = nxdef(j)
...
!
   ...
   !$acc wait(async_id)
   !$acc update device(runoff, totallp, raincu, rainlp, raincu1, rainlp1, &
   !$acc&       raincu6, rainlp6, raintot) async(async_id)
   !$acc wait(async_id)
!
   if (itaui .eq. 0) then   ! when restart, don't zero out
      ...
      !$acc wait(async_id)
      !$acc update device(rs, sld, ss, rld) async(async_id)
      !$acc wait(async_id)
   end if
!xb110>
!byl      rmr = 0.
...
!***********************************************************************
   !$acc enter data create(ptend) async(async_id)
   !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!
         ...
         do k = 1, lev
            do i = 1, nxp
               ...
               if (i .le. nxj) then
                  ...
                  vdmerd(i, k, jj) = (vt(i, k, jj) - vp(i, k, jj))/dta
      ...
      call tranrs_gpu_cuda_graph(jtrun, jtmax, nx, my, my_max, levp, polyf, weight, &
                                 ...
                                 gwk1_cg, ws_cg, wc_cg, wcc_fk_cg, fj_weight_cg)
      call trandv_gpu_cuda_graph(jtrun, jtmax, nx, my, my_max, levp, vdzonl, vdmerd, &
                                 weight, cim, onocos, poly, dpoly, vorten, divten, nsizey, &
                                 cc_cg, gwk1_cg, ws_cg, wc_cg(1, 1), wc_cg(1, 2), &
                                 wcc_fk_cg, fj_weight_cg(1, 1), fj_weight_cg(1, 2))
...
!
   ...
   if (yesdia) then
      ...
      cputime2 = mpi_wtime()
      !call nvtxEndRange
      !if (myrank .eq. 0) print 2650, cputime2-cputime1
      2650 format(' diabat time = ', f8.3, ' elapse seconds')
      ...
      !!$acc& pt, &
      !!$acc& pdot, vvel, sdpbl, &
      !!$acc& pltemp, plten, um, vm, qm, &
      !!$acc& plnow) async(async_id)
      !$acc wait(async_id)
!--------------------------------------------------------------------------------
!
! add reynolds stress
!
      ...
      if (two_loop) then
         if (mass_dp) then
            call mpe2d_unify_nx_gpu(ww1, pt)
            call tranrs1_gpu(jtrun, jtmax, nx, my, my_max, polyf, weight, ww1, &
                             plnow, nsizey, cc_cg, gwk1_cg)
         end if
         call joinrs_gpu(cc_cg, tt, dummy, dummy, dummy, &
                         nx, my_max, lev, jlistnum, 1, 1)
         call tranrs_gpu_cuda_graph(jtrun, jtmax, nx, my, my_max, levp, &
                                    polyf, weight, cc_cg, temnow, 1, nsizey, &
                                    gwk1_cg, ws_cg, wc_cg, wcc_fk_cg, fj_weight_cg)
...
!
      ...
      !                              wcc_fk_cg, fj_weight_cg(1, 1), fj_weight_cg(1, 2))
      call trandv_gpu_cuda_graph(jtrun, jtmax, nx, my, my_max, levp, &
                                 vdzonl, vdmerd, weight, cim, onocos, &
                                 polyf, dpolyf, vorten, divten, nsizey, &
                                 cc_cg, gwk1_cg, ws_cg, wc_cg(1, 1), wc_cg(1, 2), &
                                 wcc_fk_cg, fj_weight_cg(1, 1), fj_weight_cg(1, 2))
...
!
   if (ldailyFCTsst .OR. ldailyFCTicesndpt .OR. dailyClm_option .ge. 1) then
      !$acc wait(async_id)
      !$acc update self(ocean, tg, sndepth, cice) async(async_id)
      !$acc wait(async_id)
      print *, "daily entry"
      if (tau .ge. 24.) then
         do jj = 1, jlistnum
...
!                                 -2.0*tseanow(ii,jj)+tseanew(ii,jj) )
      ...
      !$acc wait(async_id)
      !$acc update device(ocean, tg, sndepth, cice) async(async_id)
      !$acc wait(async_id)
   end if
!
...
! calculate Tmax Tmin @ 2m from T2
   !$acc wait(async_id)
   !$acc update self(t2) async(async_id)
   !$acc wait(async_id)
   do jj = 1, jlistnum
      j = jlist1(jj)
      nxj = nxdef_2d(j)
...
#ifdef TIMCOMCPL
      !$acc wait(async_id)
      !$acc update self(pt, tg) async(async_id)
      !$acc wait(async_id)
...
!
         ...
         !$acc wait(async_id)
         !$acc update device(raincu, rainlp) async(async_id)
         !$acc wait(async_id)
...
!       output f006 data for FV3
      ...
      if (itau == 6) then
         ...
         !$acc wait(async_id)
         !$acc update self(ustar, srflag, q2, fm, fh, fm10, fh2) async(async_id)
         !$acc wait(async_id)
...
!               if(tau.le.192. .and. dtaup.lt.0.01)then
      ...
      if (mod(itau, 1) == 0) then
         ...
         tmin = 0.0
         !$acc wait(async_id)
         !$acc update device(raincu1, rainlp1) async(async_id)
         !$acc wait(async_id)
      end if
!
...
!       out green energy plan
      if (out_green .and. mod(itau, nint(otgreen)) == 0) then
         ...
         if (mod(itau, 6) == 0) then
            ...
            rainlp6 = 0.
         !$acc wait(async_id)
         !$acc update device(raincu6, rainlp6) async(async_id)
         !$acc wait(async_id)
         end if
      end if
!
...
!
   if (lopgsst) then
      ...
      if (lnewyear) then
         !$acc wait(async_id)
         !$acc update self(totalp, ocean, ice) async(async_id)
         !$acc wait(async_id)
         call read_opgsst(idtg1_sst, ggdef, ocean, ice)
         icurrentyear = idtg1_sst/100000000
         !$acc wait(async_id)
         !$acc update device(totalp, ocean, ice) async(async_id)
         !$acc wait(async_id)
      end if
   end if   !end lopgsst
!
...
!ch   call mpe_broadcast(istat,1,flag,mpe_integer)
   ...
   if (istat .eq. -1) then
      ...
      if (myrank .eq. 0) then
         ...
         ! ------------------------------------------------------------
         !$acc exit data delete(vorold, divold, temold, plold) async(async_id)
         !$acc exit data delete(vornow, divnow, temnow, plnow) async(async_id)
         !$acc exit data delete(rdiv, ut, vt, tt, qt, pt) async(async_id)
         !$acc exit data delete(vvel) async(async_id)
         !$acc exit data delete(phi, pk ,pk2, plt) async(async_id)
         !$acc exit data delete(ptend) async(async_id)
         ...
         !$acc& vdmerd, vdzonl, vdmerdr, vdzonlr, vdmerdg, vdzonlg, vdzonlrp, vdmerdrp, &
         !$acc& pdot, sdpbl, pltemp) async(async_id)
         !$acc exit data delete(ww1, pten, deldm, wkmf) async(async_id)
         call deallocate_spec_cg_buffer
         ! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         ...
         !! << variabels used in diabat_gpu >>
         !$acc exit data delete(o3l, dtrad, asl, atl, asl_clr, atl_clr, e, eps, &
         !$acc&     cnvcr, cnvwr, ftp, fqp, ftp1, fqp1, clds) async(async_id)
         !$acc exit data delete(dtcup, ducup, dvcup) async(async_id)
         !$acc exit data delete(stc, smc, slc, gwclim, tgclim, totalp, rs, sfalb, &
         !$acc&     sigmaf, istyp, ivegtyp, slopetyp) async(async_id)
         !$acc exit data delete(hprime_b) async(async_id)
