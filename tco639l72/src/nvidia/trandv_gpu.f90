...
subroutine trandv_gpu(jtrun, jtmax, nx, my, my_max, lev, ut, vt, w, cim &
...
!
   ...
   integer async_id
   real(kind=RTYPE) wss_local

   async_id = 1
   !$acc enter data copyin(ut, vt, w, cim, onocos, poly, dpoly, vor, div, nlist, jlist2, mtrundef) async(async_id)
...
!2dMPI
   call joinrs_gpu(cc, ut, vt, dummy, dummy, nx, my_max, lev, jlistnum, 2, 1)
...
!$omp end parallel do
   ...
   do m = 1, mlistnum
      mf = mlist(m)
      !$acc parallel loop gang private(j1, j2) async(async_id)
      do j = 1, myhalf
      ...
      if (jtrun .gt. mf) then
         ...
         !$acc parallel loop gang async(async_id) private(j)
         ...
         !$acc parallel loop gang async(async_id) private(l_fj, j)
         do l = mf, lle, 2
            l_fj = (l - mf + 2)/2
            !$acc loop vector
            do j_fj = 1, jlistnum_fj
         ...
         call dgemm_async('n', 'n', levp*2*2, llistnum_fj, jlistnum_fj, 1.0d+0, fj_wcc2, levp*2*2, &
                          fj_wd2, myhalf, 1.0d+0, fj_wss23, levp*2*2, async_id)

         call dgemm_async('n', 'n', levp*2*2, llistnum_fj, jlistnum_fj, 1.0d+0, fj_wcc3, levp*2*2, &
                          fj_wp3, myhalf, 1.0d+0, fj_wss23, levp*2*2, async_id)

         !$acc parallel loop gang async(async_id) private(l_fj)
         do l = mf, lle, 2
            l_fj = (l - mf + 2)/2
            !$acc loop vector
            do k = 1, levp*2*2
         ...
         call dgemm_async('n', 'n', levp*2*2, llistnum_fj, jlistnum_fj, 1.0d+0, fj_wcc4, levp*2*2, &
                          fj_wd4, myhalf, 1.0d+0, fj_wss45, levp*2*2, async_id)

         call dgemm_async('n', 'n', levp*2*2, llistnum_fj, jlistnum_fj, 1.0d+0, fj_wcc5, levp*2*2, &
                          fj_wp5, myhalf, 1.0d+0, fj_wss45, levp*2*2, async_id)

         !$acc parallel loop gang private(l_fj) async(async_id)
         do l = mf, lle, 2
            l_fj = (l - mf + 2)/2
            !$acc loop vector
            do k = 1, levp*2*2
      ...
      if (lchk .eq. 1) then
         ...
         do j = 1, myhalf
            if (mf .le. mtrundef(j)) then
               !$acc loop vector private(wss_local)
               do k = 1, levp*2*2
                  wss_local = wcc2(k, 1, 1, j)*wd(j, l) + wcc3(k, 1, 1, j)*wp(j, l)
                  !$acc atomic
                  wss(k, 1, 1, l) = wss(k, 1, 1, l) + wss_local
               end do
            end if
         end do
      end if
   ...
   !$acc exit data delete(wcc2, wcc3, wcc4, wcc5, wcc_fk, wd, wp, wss, jlist_fj, fj_wss23, fj_wss45, fj_wcc2, fj_wcc3, fj_wcc4, fj_wcc5, fj_wd2, fj_wp3, fj_wd4, fj_wp5) async(async_id)
   !$acc exit data copyout(vor, div) delete(ut, vt, w, cim, onocos, poly, dpoly, nlist, jlist2, mtrundef) async(async_id)
   !$acc wait(async_id)

   return
end subroutine trandv_gpu

subroutine trandv_gpu_cuda_graph(jtrun, jtmax, nx, my, my_max, lev, &
                                 ut, vt, w, cim, onocos, poly, dpoly, &
                                 vor, div, nsize, &
                                 cc, gwk1, wss, wc1, wc2, wcc_fk, fj_wp, fj_wd)
   ! Present on device: vt, ut, w, cim, onocos, poly, dpoly, div, vor
   ...
   use paramt
   use fftcom
   use spec_cuda_graph, only: fft_cg => trandv_fft_cg, lt_cg => trandv_lt_cg
   use openacc
   use cudafor
   use cublas
   ...
   integer jtrun, jtmax, nx, my, my_max, lev, nsize
   ! << output >>
   real(kind=RTYPE), dimension(lev, 2, jtrun, jtmax):: div, vor
   ! << input >>
   real(kind=RTYPE), dimension(nxp, levf, my_max)::vt, ut
   ! << const >>
   ...
   ! Present on device: cc, vt, ut, jlist1, nxjlen, nxjlen_all
   call joinrs_gpu(cc, vt, ut, dummy, dummy, nx, my_max, levf, jlistnum, 2, 1)
   ...
   do m = 1, mlistnum
      do L = 1, jtrun
         do k = 1, lev
            mf = mlist(m)
            if (L .ge. mf) then
               div(k, 1, L, m) = wss(k, 1, 1, L, m)
               div(k, 2, L, m) = wss(k, 1, 2, L, m)

               vor(k, 1, L, m) = wss(k, 2, 1, L, m)
               vor(k, 2, L, m) = wss(k, 2, 2, L, m)
            end if
         end do
      end do
   end do
   ...
   return
end subroutine trandv_gpu_cuda_graph
