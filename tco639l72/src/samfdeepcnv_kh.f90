...
!!  @{
      subroutine samfdeepcnv_kh(im,ix,km,delt,delp,prslp,psp,phil,ql, &
        ...
        do i = 1, im
          if (cnvflg(i)) then
            if(k > kbcon1(i) .and. k < ktcon(i)) then
              ...
              wc(i) = wc(i) + tem * dz
              tem=10.0*dot(i,k)*to(i,k)/po1(i,k)
              tem1=10.0*dot(i,k-1)*to(i,k-1)/po1(i,k-1)
              wbar(i)=wbar(i)+(-0.5*rd/g)*(tem+tem1)*dz 
              sumx(i) = sumx(i) + dz
            endif
          endif
        enddo
...
!> - For scale-aware parameterization, the updraft fraction (sigmagfm) is first computed as a function of the lateral entrainment rate at cloud base (see Han et al.'s (2017) \cite han_et_al_2017 equation 4 and 5), following the study by Grell and Freitas
      do i = 1, im
        if(cnvflg(i)) then
          sigmagfm(i) = wbar(i) / wc(i) !scale-aware parameterization (Kwon and Hung 2017)
