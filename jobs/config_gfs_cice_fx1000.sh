...
if [ $1 = 639  ] ; then
  ...
  MODEL_BASIC='nco=200,'
  MODLST_RES="dt=900.,hfilt=1., cgwd=2.4, cmbk=0.6, tofd=t, ksgeo=2, outdms=0, outgrb2=1,
             domfc=384., out_green=f, nmmiph=15, nmgwcv=2, two_loop=t, mom4ice=t, 
             nmcup=7, nmshl=4, factop=60., mass_dp=t, itter=1, doclx=t, spl1=10.,vd=0.00,mwhd=1.0,dorst=f,"
  DMSFLAG=GK
  NPEX=6
  NPEY=100
fi
...
#-- write out restart
cat ${GFSDIR2}/etc_4cpl/timectl > ${GFSWRK}/timectl
...
if [ ${struc} = 2cpl -o ${struc} = 2cpl_CICE ] ; then
cat > ${GFSWRK}/namlsts << EOF
&model_param
 ${MODEL_BASIC}
 lev=72,
