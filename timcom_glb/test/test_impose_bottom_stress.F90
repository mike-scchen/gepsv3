program test_impose_bottom_stress
  implicit none

  call timcom_initialize_gpu
  call impose_bottom_stress_unit
  call timcom_finalize_gpu

end program test_impose_bottom_stress

subroutine impose_bottom_stress_unit
  use timcom_drv, only: impose_bottom_stress
  use timcom_drv_gpu, only: impose_bottom_stress_gpu
  use hyperlink, only: u1, u2, v1, v2, dt, odz
  use hyperlink, only: nx, ny, nz, nzf, in, kb
  implicit none
...
!  integer, parameter :: async_id = -1 !測時間時要給-1
  ...
  u2=u2_tmp
  v2=v2_tmp
  !$acc data async(async_id) copy(u2(1:nx,1:ny,:), v2(1:nx,1:ny,:)) &
  !$acc& copyin(nx, ny, dt, odz(:), in(1:nx,1:ny,1:nzf)) &
  !$acc& copyin(u1(1:nx,1:ny,:),v1(1:nx,1:ny,:),kb(1:nx,1:ny))
  gt = 0.
  do i = 1, num_steps
    call cpu_time(time1)
...
end subroutine impose_bottom_stress_unit
