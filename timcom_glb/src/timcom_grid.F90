module timcom_grid
  use timcom_const, only: r8, r4
  use timcom_comm,  only: comm_panel
  implicit none
...
subroutine allocate_grid_var(grd)
  ...
  call allocate_r8_4d(grd%t_clim, grd%nx, grd%ny, grd%nz, 12, 0, 0)
  call allocate_r8_4d(grd%s_clim, grd%nx, grd%ny, grd%nz, 12, 0, 0)
  call allocate_r8_2d(grd%t_nudge, grd%nx, grd%ny, 0, 0)
  call allocate_r8_2d(grd%s_nudge, grd%nx, grd%ny, 0, 0)
  call allocate_r8_2d(grd%t_da,    grd%nx, grd%ny, 0, 0)
  ...
  call allocate_r8_2d(grd%vice, grd%nx, grd%ny, 0, 0)
  call allocate_r8_2d(grd%vsno, grd%nx, grd%ny, 0, 0)
  call allocate_r8_2d(grd%melt, grd%nx, grd%ny, 0, 0)
  call allocate_r8_2d(grd%salt, grd%nx, grd%ny, 0, 0)
  ...
  call allocate_r8_2d(grd%tgfs,  grd%nx, grd%ny, 0, 0)
...
subroutine allocate_r8_4d(var, ni, nj, nk, nl, nigh, njgh)
  implicit none

  real(r8), pointer :: var(:,:,:,:)
  integer :: ni, nj, nk, nl, nigh ,njgh

  allocate(var(1-nigh:ni+nigh, 1-njgh:nj+njgh, nk, nl))
  var = 0.d0
end subroutine allocate_r8_4d
