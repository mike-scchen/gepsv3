module timcom_init
  use timcom_const
  use timcom_comm
  use timcom_grid, only: ocn_grid, ocn_panel
  use hyperlink
  implicit none
...
#else
subroutine grid_init(grid_list)
#endif
  use timcom_general, only: member, ocn_grid_num, str_split
  use timcom_grid,    only: allocate_grid_var
  use timcom_kpp,     only: init_vmix_kpp
  use timcom_cplmct,  only: ocn_cpl_init
  implicit none
...
subroutine get_grid_inp_nml(nml_file)
  ...
  namelist /phy_info/    opt_windmix, opt_arbr_p0, dm0, opt_da
  namelist /solver_info/ opt_solver, threshold_bicg, threshold_p0, max_iter_p0, &
  ...
  if(myid_timcom .eq. rootid) then
    ...
    call comm_write_log_info(fid_log, log_info)
    write(log_info, *) '        dt and odt: ', 0.5d0*dt, 2.d0*odt
    call comm_write_log_info(fid_log, log_info)
  end if
end subroutine get_grid_inp_nml

subroutine get_grid_indata
  use timcom_pncio, only: get_indata_pncio
  implicit none
  ...
  call ghost_cell_exch(m_comm_cart, i2type3d, nbid, ndim, in, symm_np)
  call ghost_cell_exch(m_comm_cart, i2type2d, nbid, ndim, kb, symm_np)
  ...
  yv_bgn = 1
  yv_end = nyf
  if(.not. peri_y .and. .not. symm_np) then
...
#else
        temp = dm0*(dexp(-1.d-3*(2-j-jt)**2)+dexp(-1.d-3*(ny_grid-jt-j)**2))      ! north & south poles
        scr(i,j,k) = dm0*dexp(-1.d-3*(y_grid(j)**2)) + temp
#endif
      end do
    end do
  end do

  if(y_grid(1) .gt. 83.d0) scr = scr*1000.d0

  call ghost_cell_exch(m_comm_cart, r8type3d, nbid, ndim, scr, symm_np)

#ifdef cpl
  scr = merge(scr*1.d2, scr, in.eq.0)
  scr2 = scr
...
subroutine set_grid_solver
  ...
  real(r8) :: dzx, dzyb, dzyt, tmp  ! NTU 20250417 move gamma_tmp to module timcom_solver
  character(len=256) :: log_info
...
! NTU 20250417 M. Hsieh  Initialize variables of module timcom_solver

  if(myid_timcom .eq. rootid) then
     write(log_info,*) "[INFO]: opt_solver = ", opt_solver, "pcsi_cuda_graph = ", pcsi_cuda_graph
     print *, "opt_solver = ", opt_solver, "pcsi_cuda_graph = ", pcsi_cuda_graph
     call comm_write_log_info(fid_log, log_info)
  end if

  PcsiMinEig =  1.0d5
  PcsiMaxEig = -1.0d5
  if ( opt_solver == solver_PCSI )  &
     call PcsiLanczos(nx, ny, al, ab, ac, ar, at, &
                              cl, cb, cc, cr, ct, &
                      PcsiMaxEig, PcsiMinEig, ierr)
  if(myid_timcom .eq. rootid) then
     write(log_info,*) &
        '[INFO]: max, min eigenvalues are ', PcsiMaxEig, ' and ', PcsiMinEig
     call comm_write_log_info(fid_log, log_info)
  end if
...
subroutine set_initial_condition
  use timcom_general, only: restart
  implicit none
...
subroutine set_initial_condition_restart
  use timcom_general, only: member, path_restart, timer_drv
  use timcom_pncio, only: get_restart_pncio, put_ic_pncio
  implicit none
...
subroutine set_initial_condition_newrun
  ...
  use timcom_pncio, only: get_ssh_pncio, get_ic_pncio, put_ic_pncio
  implicit none

  character(len=128) :: ic_file, ic_ssh_file
  character(len=256) :: log_info
  integer :: i, j, k, ierr
  real(r8) :: tmpd, psm, tmp
...
subroutine set_ts_nudging
  use timcom_pncio, only: get_ts_climate, get_da_data
  implicit none
