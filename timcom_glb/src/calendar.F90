...
!
      ...
      function mjd2_noleap(y,m,d,h,minut)
      implicit none
      integer d,m,y,h,minut
      real(r8) :: mjd2_noleap
      integer(2), parameter :: days_of_mon_end(0:12) = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365]
! Mathematicians and programmers have naturally 
! interested themselves in mathematical and computational 
! algorithms to convert between Julian day numbers and 
! Gregorian dates. The following conversion algorithm is due 
...
! No-leap ModifiedJulianDay = 0 for 1958-01-01 CE.

      mjd2_noleap = (y-1958)*365 + days_of_mon_end(m-1) + (d-1) + h/24.d0 + minut/(24*60.d0)

      end function mjd2_noleap
...
!
      ...
      subroutine gregd2_noleap(mjd,y,m,d,h,minut)
      implicit none
      real(r8), intent(in) :: mjd
      real(r8) :: dd,temp1,temp2
      integer, intent(out) :: d,m,y,h,minut

      integer              :: i
      real(r8), parameter :: days_of_mon_end(0:12) = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365]

! Converting from the modified Julian day number to the Gregorian
! date is performed thus:

!date
        y = floor(mjd/365.d0) + 1958
        dd = dmod(mjd, 365.d0)

        do i = 1, 12
          if(days_of_mon_end(i) .gt. dd .and. days_of_mon_end(i-1) .le. dd) m = i
        end do
        d = floor(dd) - days_of_mon_end(m-1) + 1
!time in the day
        temp1=mjd-floor(mjd)
        temp2=temp1*24
        h=nint(temp2)
