module timcom_general
  ...
  type :: timer_panel
    ...
    real(r8) :: mjd1, mjd2, dt_mjd, syng_mjd, shift_mjd
...
subroutine get_namelist_run(grid_list)
  ...
  #endif
#else
  timer_drv%base_mjd = mjd2_noleap(timer_drv%greg1(1), timer_drv%greg1(2), 1, 0, 0)
  timer_drv%init_mjd = mjd2_noleap(timer_drv%greg1(1), timer_drv%greg1(2), timer_drv%greg1(3), timer_drv%greg1(4), 0) - timer_drv%base_mjd
  timer_drv%dest_mjd = mjd2_noleap(timer_drv%greg2(1), timer_drv%greg2(2), timer_drv%greg2(3), timer_drv%greg2(4), 0) - timer_drv%base_mjd
#endif
  timer_drv%mjd1 = timer_drv%init_mjd
  timer_drv%mjd2 = timer_drv%init_mjd
#ifdef cpl
  #ifdef clm_r
  timer_drv%shift_mjd = timer_drv%base_mjd - mjd2(timer_drv%greg1(1), 1, 1, 0, 0)
  #endif
#else
  timer_drv%shift_mjd = timer_drv%base_mjd - mjd2_noleap(timer_drv%greg1(1), 1, 1, 0, 0)
#endif
 ...
 #else
    write(log_info,'(a,f12.4,a,i4,a,i2,a)') 'init mjd: ' , timer_drv%init_mjd, ' days since ', timer_drv%greg1(1),'-',timer_drv%greg1(2),'-01 00:00:00'
 #endif
#else
    write(log_info,'(a,f12.4,a,i4,a,i2,a)') 'init mjd: ' , timer_drv%init_mjd, ' days since ', timer_drv%greg1(1),'-',timer_drv%greg1(2),'-01 00:00:00'
#endif
 ...
 #else
    write(log_info,'(a,f12.4,a,i4,a,i2,a)') 'dest mjd: ' , timer_drv%dest_mjd, ' days since ', timer_drv%greg1(1),'-',timer_drv%greg1(2),'-01 00:00:00'
 #endif
#else
    write(log_info,'(a,f12.4,a,i4,a,i2,a)') 'dest mjd: ' , timer_drv%dest_mjd, ' days since ', timer_drv%greg1(1),'-',timer_drv%greg1(2),'-01 00:00:00'
#endif
    call comm_write_log_info(fid_log, log_info)
    write(log_info,'(a,i6, a)') 'duration: ' , duration, trim(duration_unit)
    call comm_write_log_info(fid_log, log_info)
...
subroutine dtg2greg(dtg, greg)
 ...
 #else
  subroutine integral_var2d(var2d, mjd1_drv, mjd2_drv, shift_mjd, opt)
 #endif
#else
 subroutine integral_var2d(var2d, mjd1_drv, mjd2_drv, shift_mjd, opt)
#endif
  use timcom_ncio, only: get_ncdata_var
  implicit none

  type(data_panel) :: var2d
  real(r8) :: mjd1_drv, mjd2_drv, mjd1, mjd2, shift_mjd
#ifdef cpl
 #ifndef clm_r
  ...
  integer :: opt
#endif
  real(r8) :: dt_var2d, tmp1, tmp2, odt_mjd
  ...
  #endif
#else
  mjd1 = 0.5d0*(mjd2_drv + mjd1_drv)
  if(opt .eq. 1 .or. opt .eq. 4) then     
    mjd1 = mjd1*24.d0 
  end if
#endif
 ...
 #ifdef clm_r
  if(opt .eq. 2) then
    mjd1 = mjd1 + shift_mjd +1
  end if
  if(mjd1 .gt. 365.5d0) mjd1=mjd1-365.d0
 #endif
#else
  if(opt .eq. 2) then
    mjd1 = mjd1 + shift_mjd
...
!  if(myid_timcom .eq. rootid)  write(*,*) "CHECK datm file:", trim(var2d%fname)
!  if(myid_timcom .eq. rootid)  write(*,*) trim(var2d%fvara), " CHECK datm time:", var2d%time
!  if(myid_timcom .eq. rootid)  write(*,*) trim(var2d%fvara), " CHECK datm lon:", var2d%lon
!  if(myid_timcom .eq. rootid)  write(*,*) trim(var2d%fvara), " CHECK datm lat:", var2d%lat
! if(myid_timcom .eq. rootid) then
  do t = 1, var2d%nd(1)-1
 ...
 #else
  if(mjdt1(1) .ne. var2d%t(1) .or. mjdt1(2) .ne. var2d%t(2)) then
 #endif
#else
  if(mjdt1(1) .ne. var2d%t(1) .or. mjdt1(2) .ne. var2d%t(2)) then 
#endif
...
!      var2d%src(:,:,1) = anint(var2d%src(:,:,1)*1000000)/1000000
    end if
    var2d%t(2) = mjdt1(2)
    call get_ncdata_var(var2d, mjdt1(2), var2d%src(:,:,2))
!    var2d%src(:,:,2) = anint(var2d%src(:,:,2)*1000000)/1000000
  end if

  tmp1 = (var2d%time(mjdt1(2))-mjd1)
!  tmp1 = anint(tmp1*1000000)/1000000
#ifdef cpl
 #ifndef clm_r
  ...
  if(mjdt1(1) .lt. mjdt1(2)) then
    ...
    tmp1 = anint(tmp1*1000000)/1000000
    tmp2 = 1.d0 - tmp1
    var2d%srcm = tmp1*var2d%src(:,:,1)+tmp2*var2d%src(:,:,2)
  elseif( mjdt1(1) .eq. 1) then
    var2d%srcm = var2d%src(:,:,1)
  else
    var2d%srcm = var2d%src(:,:,2)
  end if
 #endif
#else
  if(mjdt1(1) .lt. mjdt1(2)) then
...
!  call MPI_BCAST(mjdt2, 2, MPI_INT, 0, m_comm_timcom, ierr)
 ...
 #else
  if(myid_timcom .eq. rootid) then
     write(log_info,'(a,a,a,f15.8,a3,f15.8,a5,1f15.8,4I4, 3e22.15)') &
       "[INFO]: integral ", trim(var2d%fvara), " from:", mjd1_drv, " to", mjd2_drv, " days", mjd1, mjdt1, var2d%t, var2d%srcm(566,491), var2d%src(566,491,1), var2d%src(566,491,2)
 #endif
#else
  if(myid_timcom .eq. rootid) then
     write(log_info,'(a,a,a,f15.8,a3,f15.8,a5,1f15.8,4I4, 3e22.15)') &
