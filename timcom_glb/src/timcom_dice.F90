module timcom_dice
  ...
  use timcom_general, only: ice_grid_num, timer_panel
  implicit none
...
subroutine dice_exec(timer)
  ...
  !call integral_var2d(ice_grid(i)%flx(n), timer%syng_mjd, timer%syng_mjd+timer%dt_mjd, timer%shift_mjd, 2)
 #else 
  call integral_var2d(ice_grid(i)%flx(n), timer%syng_mjd, timer%syng_mjd+timer%dt_mjd, timer%shift_mjd, 2)
 #endif
#else
  call integral_var2d(ice_grid(i)%flx(n), timer%mjd1, timer%mjd2, timer%shift_mjd, 2)
#endif      
      end if
    end do
  end do
...
#else
  ocn_grid(i)%ifrc = dble(ice_grid(i)%flx(idx_i2x_ifrc)%srcm(x1:x2,y1:y2))
#endif

end subroutine dice_to_timcom

end module timcom_dice
