module timcom_drof
  ...
  use timcom_general, only: rof_grid_num, timer_panel
  implicit none
...
subroutine drof_init(grid_list)
  use timcom_general, only: str_split, member
  implicit none
...
subroutine drof_exec(timer)
 ...
 #else
  call integral_var2d(rof_grid(i)%flx(n), timer%mjd1, timer%mjd2, timer%shift_mjd, 4)
!  call integral_var2d(rof_grid(i)%flx(n), timer%syng_mjd, timer%syng_mjd+timer%dt_mjd, timer%shift_mjd, 4)
 #endif
#else
  call integral_var2d(rof_grid(i)%flx(n), timer%mjd1, timer%mjd2, timer%shift_mjd, 4)
#endif
...
#ifdef cpl
  ocn_grid(i)%roff = rof_grid(i)%flx(idx_r2x_roff)%srcm(x1:x2,y1:y2)
#else
  ocn_grid(i)%roff = dble(rof_grid(i)%flx(idx_r2x_roff)%srcm(x1:x2,y1:y2))
#endif 

end subroutine drof_to_timcom

end module timcom_drof
