...
! tidal mixing
  ...
  if(lhoriz_varying_bckgrnd) then
    ...
    do j = 1, ny
      do i =1, nx
        bckgrnd_vdc_psis   = bckgrnd_vdc_psim*dexp(-(0.4d0*(y_grid(j)+28.9d0))**2)
        bckgrnd_vdc_psin   = bckgrnd_vdc_psim*dexp(-(0.4d0*(y_grid(j)-28.9d0))**2)
...
! NEED TO CHECK THE UNIT OF STF/DZ*VDC again
 ...
 #else
  subroutine kppglo(itf, u2, v2, t2, s2, rho, trans, &
                  stf, shf_qsw, smft, vbk, hbk, &
                  add, vdc, vvc, ev, hv, kpp_src, kpp_hblt)
 #endif
#else
...
!-----------------------------------------------------------------------
  if(present(smft)) then
    ustar = dsqrt(dsqrt(smft(:,:,1)**2 + smft(:,:,2)**2))
  else
    work = dsqrt(dsqrt(smf(:,:,1)**2 + smf(:,:,2)**2))
    ustar = work
  end if
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
    b_frqncy = dsqrt( &
               0.5d0*(dbloc(:,:,kl) + dabs(dbloc(:,:,kl)) + eps2)/  &
               (zgrid(kl)-zgrid(kl+1)) )
